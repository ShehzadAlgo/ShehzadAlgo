...
- `/usage cost` in chats: local cost summary aggregated from ShehzadAlgo session logs.
- CLI: `shehzadalgo status --usage` prints a full per-provider breakdown.
- CLI: `shehzadalgo channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
