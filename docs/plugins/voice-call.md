...
read_when:
  - You want to place an outbound voice call from ShehzadAlgo
...
Voice calls for ShehzadAlgo via a plugin. Supports outbound notifications and
...
- Use `shehzadalgo voicecall ...` or the `voice_call` tool
...
```bash
shehzadalgo plugins install ./extensions/voice-call
