...
read_when:
  - Designing workspace memory (~/.shehzadalgo/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep ShehzadAlgo integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.shehzadalgo/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.shehzadalgo/workspace` as canonical human-readable memory.
...
```
~/.shehzadalgo/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in ShehzadAlgo**, but keep a separable core library.

### Why integrate into ShehzadAlgo?

- ShehzadAlgo already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `shehzadalgo memory recall "…" --k 25 --since 30d`
  - `shehzadalgo memory reflect --since 7d`
...
Pragmatic take for `~/.shehzadalgo/workspace`:
