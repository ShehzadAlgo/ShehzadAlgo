---
summary: "Use Venice AI privacy-focused models in ShehzadAlgo"
read_when:
  - You want privacy-focused inference in ShehzadAlgo
...
```bash
shehzadalgo onboard --auth-choice venice-api-key
```
...
```bash
shehzadalgo chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, ShehzadAlgo shows all available Venice models. Pick based on your needs:
...
```bash
shehzadalgo models set venice/claude-opus-45
shehzadalgo models set venice/llama-3.3-70b
```

List all available models:

```bash
shehzadalgo models list | grep venice
```

## Configure via `shehzadalgo configure`

1. Run `shehzadalgo configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
ShehzadAlgo automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
shehzadalgo chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
shehzadalgo chat --model venice/claude-opus-45

# Use uncensored model
shehzadalgo chat --model venice/venice-uncensored

# Use vision model with image
shehzadalgo chat --model venice/qwen3-vl-235b-a22b

# Use coding model
shehzadalgo chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `shehzadalgo models list` to see currently available models. Some models may be temporarily offline.
