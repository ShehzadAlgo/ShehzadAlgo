---
summary: "Use MiniMax M2.1 in ShehzadAlgo"
read_when:
  - You want MiniMax models in ShehzadAlgo
...
```bash
shehzadalgo plugins enable minimax-portal-auth  # skip if already loaded.
shehzadalgo gateway restart  # restart if gateway is already running
shehzadalgo onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/Shehzadwattu523/ShehzadAlgo/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `shehzadalgo.json`:
...
- Use `shehzadalgo models list` and `shehzadalgo models set minimax/MiniMax-M2.1` to switch.
...
- Running `shehzadalgo configure` and selecting **MiniMax M2.1**, or
