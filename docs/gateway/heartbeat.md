...
- During heartbeat runs, ShehzadAlgo treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `shehzadalgo sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, ShehzadAlgo skips the heartbeat run entirely (no model call).
...
```bash
shehzadalgo system event --text "Check for urgent follow-ups" --mode now
```
