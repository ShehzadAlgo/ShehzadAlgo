...
read_when:
  - Running ShehzadAlgo behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of ShehzadAlgo
...
- You run ShehzadAlgo behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. ShehzadAlgo checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. ShehzadAlgo extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://shehzadalgo.example.com
    to: http://shehzadalgo-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
shehzadalgo.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy shehzadalgo:18789 {
...
location / {
    ...
    proxy_pass http://shehzadalgo:18789;
...
`shehzadalgo security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update ShehzadAlgo config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `shehzadalgo security audit` and review findings
