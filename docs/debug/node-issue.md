...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ShehzadAlgo
...
Running ShehzadAlgo via Node with `tsx` fails at startup with:

```
[shehzadalgo] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `shehzadalgo status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch shehzadalgo.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node shehzadalgo.mjs status` works on Node 25.
