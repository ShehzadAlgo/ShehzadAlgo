...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects ShehzadAlgo to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
shehzadalgo plugins install @shehzadalgo/feishu
```

Local checkout (when running from a git repo):

```bash
shehzadalgo plugins install ./extensions/feishu
```
...
If you just installed ShehzadAlgo, run the wizard:
...
2. Configuring app credentials in ShehzadAlgo
...
1. You already ran `shehzadalgo channels add` for Feishu
2. The gateway is running (`shehzadalgo gateway status`)
...
Edit `~/.shehzadalgo/shehzadalgo.json`:
...
```bash
shehzadalgo pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  shehzadalgo pairing list feishu
  shehzadalgo pairing approve feishu <CODE>
  ```
...
2. Run `shehzadalgo logs --follow` and look for `chat_id`
...
2. Run `shehzadalgo logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `shehzadalgo gateway status`  | Show gateway status           |
| `shehzadalgo gateway install` | Install/start gateway service |
| `shehzadalgo gateway stop`    | Stop gateway service          |
| `shehzadalgo gateway restart` | Restart gateway service       |
| `shehzadalgo logs --follow`   | Tail gateway logs             |
...
4. Check logs: `shehzadalgo logs --follow`
...
5. Ensure the gateway is running: `shehzadalgo gateway status`
6. Check logs: `shehzadalgo logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.shehzadalgo/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.shehzadalgo/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
