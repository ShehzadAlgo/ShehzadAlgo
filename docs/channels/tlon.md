...
Tlon is a decentralized messenger built on Urbit. ShehzadAlgo connects to your Urbit ship and can
...
```bash
shehzadalgo plugins install @shehzadalgo/tlon
```

Local checkout (when running from a git repo):

```bash
shehzadalgo plugins install ./extensions/tlon
```
...
By default, ShehzadAlgo blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `shehzadalgo message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, ShehzadAlgo replies in-thread.
