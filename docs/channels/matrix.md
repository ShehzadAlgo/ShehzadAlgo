...
Matrix is an open, decentralized messaging protocol. ShehzadAlgo connects as a Matrix **user**
...
```bash
shehzadalgo plugins install @shehzadalgo/matrix
```

Local checkout (when running from a git repo):

```bash
shehzadalgo plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `shehzadalgo plugins install @shehzadalgo/matrix`
   - From a local checkout: `shehzadalgo plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: ShehzadAlgo calls the same
     login endpoint, stores the access token in `~/.shehzadalgo/credentials/matrix/credentials.json`,
...
- On first connection, ShehzadAlgo requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  ShehzadAlgo logs a warning.
...
`~/.shehzadalgo/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `shehzadalgo pairing list matrix`
  - `shehzadalgo pairing approve matrix <CODE>`
...
- On startup, ShehzadAlgo resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
