...
read_when:
  - Setting up Twitch chat integration for ShehzadAlgo
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. ShehzadAlgo connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
shehzadalgo plugins install @shehzadalgo/twitch
```

Local checkout (when running from a git repo):

```bash
shehzadalgo plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SHEHZADALGO_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "shehzadalgo", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SHEHZADALGO_TWITCH_ACCESS_TOKEN env var)
...
```bash
SHEHZADALGO_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
