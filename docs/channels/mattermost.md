---
summary: "Mattermost bot setup and ShehzadAlgo config"
read_when:
...
```bash
shehzadalgo plugins install @shehzadalgo/mattermost
```

Local checkout (when running from a git repo):

```bash
shehzadalgo plugins install ./extensions/mattermost
```
...
4. Configure ShehzadAlgo and start the gateway.
...
- Approve via:
  - `shehzadalgo pairing list mattermost`
  - `shehzadalgo pairing approve mattermost <CODE>`
...
Use these target formats with `shehzadalgo message send` or cron/webhooks:
