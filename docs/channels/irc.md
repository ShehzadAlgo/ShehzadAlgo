---
title: IRC
description: Connect ShehzadAlgo to IRC channels and direct messages.
---

Use IRC when you want ShehzadAlgo in classic channels (`#room`) and direct messages.
IRC ships as an extension plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.shehzadalgo/shehzadalgo.json`.
2. Set at least:
...
```json
{
  "channels": {
    "irc": {
      ...
      "port": 6697,
      "tls": true,
      "nick": "shehzadalgo-bot",
      "channels": ["#shehzadalgo"]
    }
  }
}
```
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, ShehzadAlgo defaults to **mention-gating** in group contexts.
