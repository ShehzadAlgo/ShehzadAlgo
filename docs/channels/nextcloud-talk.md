...
```bash
shehzadalgo plugins install @shehzadalgo/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
shehzadalgo plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "ShehzadAlgo" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `shehzadalgo pairing list nextcloud-talk`
  - `shehzadalgo pairing approve nextcloud-talk <CODE>`
