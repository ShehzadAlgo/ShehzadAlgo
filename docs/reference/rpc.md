...
ShehzadAlgo integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- ShehzadAlgo owns lifecycle when `channels.signal.autoStart=true`.
...
- ShehzadAlgo spawns `imsg rpc` as a child process (legacy iMessage integration).
