...
ShehzadAlgo features that can generate provider usage or paid API calls.
...
- `shehzadalgo status --usage` and `shehzadalgo channels list` show provider **usage windows**
...
ShehzadAlgo can pick up credentials from:
...
- `shehzadalgo status --usage`
- `shehzadalgo models status --json`
...
`shehzadalgo models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
