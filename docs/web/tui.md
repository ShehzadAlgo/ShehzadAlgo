...
```bash
shehzadalgo tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `shehzadalgo tui --deliver`
...
- Check the Gateway logs: `shehzadalgo logs --follow`.
- Confirm the agent can run: `shehzadalgo status` and `shehzadalgo models status`.
...
- No agents in picker: check `shehzadalgo agents list` and your routing config.
