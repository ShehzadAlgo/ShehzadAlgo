...
Deploy ShehzadAlgo on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/shehzadalgo` — Control UI
...
- `SHEHZADALGO_STATE_DIR=/data/.shehzadalgo` (recommended)
- `SHEHZADALGO_WORKSPACE_DIR=/data/workspace` (recommended)
- `SHEHZADALGO_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your ShehzadAlgo state + workspace so you can migrate to another host without losing config or memory.
