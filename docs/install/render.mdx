...
Deploy ShehzadAlgo on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/Shehzadwattu523/ShehzadAlgo)
...
repository configures everything needed to run ShehzadAlgo:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SHEHZADALGO_STATE_DIR
        value: /data/.shehzadalgo
      - key: SHEHZADALGO_WORKSPACE_DIR
        value: /data/workspace
      - key: SHEHZADALGO_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: shehzadalgo-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/shehzadalgo`.
...
If you use the original ShehzadAlgo repository, Render will not auto-deploy your ShehzadAlgo. To update it, run a manual Blueprint sync from the dashboard.
...
For ShehzadAlgo, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any ShehzadAlgo host.
