...
- Publish as `shehzadalgo/plugin-sdk` (or export from core under `shehzadalgo/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ShehzadAlgoPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ShehzadAlgoConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ShehzadAlgoConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ShehzadAlgoConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ShehzadAlgoConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ShehzadAlgoConfig): string;
  };
};
```
...
- Introduce `shehzadalgo/plugin-sdk`.
- Add `api.runtime` to `ShehzadAlgoPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `shehzadalgoRuntime: ">=2026.2.0"`).
