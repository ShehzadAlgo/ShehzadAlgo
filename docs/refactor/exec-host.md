...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.shehzadalgo/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.shehzadalgo/node.json` (node id + pairing token).
...
Path: `~/.shehzadalgo/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.shehzadalgo/exec-approvals.sock",
...
- Unix socket at `~/.shehzadalgo/exec-approvals.sock` (0600).
