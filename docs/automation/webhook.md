...
read_when:
  ...
  - Wiring external systems into ShehzadAlgo
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your ShehzadAlgo config directory (typically `~/.shehzadalgo/hooks/transforms`).
...
- `shehzadalgo webhooks gmail setup` writes `hooks.gmail` config for `shehzadalgo webhooks gmail run`.
