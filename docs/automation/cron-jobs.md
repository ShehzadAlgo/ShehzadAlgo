...
- Jobs persist under `~/.shehzadalgo/cron/` so restarts don’t lose schedules.
...
shehzadalgo cron run <job-id>
shehzadalgo cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.shehzadalgo/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `shehzadalgo cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, ShehzadAlgo defaults to `announce`.
...
- Job store: `~/.shehzadalgo/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.shehzadalgo/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.shehzadalgo/cron/jobs.json",
...
shehzadalgo cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
shehzadalgo cron edit <jobId> --agent ops
shehzadalgo cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
shehzadalgo cron run <jobId>
shehzadalgo cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
shehzadalgo cron edit <jobId> \
...
```bash
shehzadalgo system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`shehzadalgo system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SHEHZADALGO_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- ShehzadAlgo applies exponential retry backoff for recurring jobs after consecutive errors:
