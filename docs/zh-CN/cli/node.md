...
summary: "`shehzadalgo node` 的 CLI 参考（无头节点主机）"
title: node
x-i18n:
...
使用 `shehzadalgo node run` 运行前台节点主机（无服务）。
...
- `shehzadalgo approvals --node <id|name|ip>`（从 Gateway 网关编辑）
