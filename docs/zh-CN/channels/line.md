...
LINE 通过 LINE Messaging API 连接到 ShehzadAlgo。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
shehzadalgo plugins install @shehzadalgo/line
```

本地检出（从 git 仓库运行时）：

```bash
shehzadalgo plugins install ./extensions/line
```
...
```bash
shehzadalgo pairing list line
shehzadalgo pairing approve line <CODE>
```
