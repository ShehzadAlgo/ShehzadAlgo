...
你可以在 `~/.shehzadalgo/shehzadalgo.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/shehzadalgo.log"
  }
}
```
...
```bash
shehzadalgo channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.shehzadalgo/shehzadalgo.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/shehzadalgo/shehzadalgo-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- ShehzadAlgo 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
SHEHZADALGO_DIAGNOSTICS=telegram.http,telegram.payload
```
...
```json
{
  ...
  "diagnostics": {
    ...
    "otel": {
      ...
      "serviceName": "shehzadalgo-gateway",
      "traces": true,
      "metrics": true,
      "logs": true,
      "sampleRate": 0.2,
...
- 你也可以使用 `shehzadalgo plugins enable diagnostics-otel` 启用插件。
...
- `shehzadalgo.tokens`（计数器，属性：`shehzadalgo.token`、`shehzadalgo.channel`、`shehzadalgo.provider`、`shehzadalgo.model`）
- `shehzadalgo.cost.usd`（计数器，属性：`shehzadalgo.channel`、`shehzadalgo.provider`、`shehzadalgo.model`）
- `shehzadalgo.run.duration_ms`（直方图，属性：`shehzadalgo.channel`、`shehzadalgo.provider`、`shehzadalgo.model`）
- `shehzadalgo.context.tokens`（直方图，属性：`shehzadalgo.context`、`shehzadalgo.channel`、`shehzadalgo.provider`、`shehzadalgo.model`）

消息流：

- `shehzadalgo.webhook.received`（计数器，属性：`shehzadalgo.channel`、`shehzadalgo.webhook`）
- `shehzadalgo.webhook.error`（计数器，属性：`shehzadalgo.channel`、`shehzadalgo.webhook`）
- `shehzadalgo.webhook.duration_ms`（直方图，属性：`shehzadalgo.channel`、`shehzadalgo.webhook`）
- `shehzadalgo.message.queued`（计数器，属性：`shehzadalgo.channel`、`shehzadalgo.source`）
- `shehzadalgo.message.processed`（计数器，属性：`shehzadalgo.channel`、`shehzadalgo.outcome`）
- `shehzadalgo.message.duration_ms`（直方图，属性：`shehzadalgo.channel`、`shehzadalgo.outcome`）

队列 + 会话：

- `shehzadalgo.queue.lane.enqueue`（计数器，属性：`shehzadalgo.lane`）
- `shehzadalgo.queue.lane.dequeue`（计数器，属性：`shehzadalgo.lane`）
- `shehzadalgo.queue.depth`（直方图，属性：`shehzadalgo.lane` 或 `shehzadalgo.channel=heartbeat`）
- `shehzadalgo.queue.wait_ms`（直方图，属性：`shehzadalgo.lane`）
- `shehzadalgo.session.state`（计数器，属性：`shehzadalgo.state`、`shehzadalgo.reason`）
- `shehzadalgo.session.stuck`（计数器，属性：`shehzadalgo.state`）
- `shehzadalgo.session.stuck_age_ms`（直方图，属性：`shehzadalgo.state`）
- `shehzadalgo.run.attempt`（计数器，属性：`shehzadalgo.attempt`）

### 导出的 span（名称 + 关键属性）

- `shehzadalgo.model.usage`
  - `shehzadalgo.channel`、`shehzadalgo.provider`、`shehzadalgo.model`
  - `shehzadalgo.sessionKey`、`shehzadalgo.sessionId`
  - `shehzadalgo.tokens.*`（input/output/cache_read/cache_write/total）
- `shehzadalgo.webhook.processed`
  - `shehzadalgo.channel`、`shehzadalgo.webhook`、`shehzadalgo.chatId`
- `shehzadalgo.webhook.error`
  - `shehzadalgo.channel`、`shehzadalgo.webhook`、`shehzadalgo.chatId`、`shehzadalgo.error`
- `shehzadalgo.message.processed`
  - `shehzadalgo.channel`、`shehzadalgo.outcome`、`shehzadalgo.chatId`、`shehzadalgo.messageId`、`shehzadalgo.sessionKey`、`shehzadalgo.sessionId`、`shehzadalgo.reason`
- `shehzadalgo.session.stuck`
  - `shehzadalgo.state`、`shehzadalgo.ageMs`、`shehzadalgo.queueDepth`、`shehzadalgo.sessionKey`、`shehzadalgo.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `shehzadalgo doctor`。
