...
read_when:
  ...
  - 你想了解浏览器接管的安全影响
summary: Chrome 扩展：让 ShehzadAlgo 驱动你现有的 Chrome 标签页
title: Chrome 扩展
x-i18n:
...
ShehzadAlgo Chrome 扩展让智能体控制你**现有的 Chrome 标签页**（你的正常 Chrome 窗口），而不是启动一个单独的 shehzadalgo 管理的 Chrome 配置文件。
...
然后 ShehzadAlgo 通过正常的 `browser` 工具界面控制附加的标签页（选择正确的配置文件）。
...
扩展作为静态文件包含在 ShehzadAlgo 发布版（npm 包）中。没有单独的"构建"步骤。

升级 ShehzadAlgo 后：

- 重新运行 `shehzadalgo browser extension install` 以刷新 ShehzadAlgo 状态目录下的已安装文件。
- Chrome → `chrome://extensions` → 点击扩展上的"重新加载"。

## 使用它（无需额外配置）

ShehzadAlgo 附带一个名为 `chrome` 的内置浏览器配置文件，它指向默认端口上的扩展中继。
...
调试：`shehzadalgo sandbox explain`
...
`shehzadalgo browser extension path` 打印包含扩展文件的**已安装**磁盘目录。

CLI 有意**不**打印 `node_modules` 路径。始终先运行 `shehzadalgo browser extension install` 将扩展复制到 ShehzadAlgo 状态目录下的稳定位置。
...
- 扩展使用 Chrome 的调试器 API（`chrome.debugger`）。附加时，模型可以：
  ...
  - 读取页面内容
  - 访问标签页已登录会话可以访问的任何内容
- **这不像**专用的 shehzadalgo 管理配置文件那样隔离。
