---
summary: "Chrome extension: let ShehzadAlgo drive your existing Chrome tab"
read_when:
...
The ShehzadAlgo Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate shehzadalgo-managed Chrome profile.
...
ShehzadAlgo then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the ShehzadAlgo release (npm package) as static files. There is no separate “build” step.

After upgrading ShehzadAlgo:

- Re-run `shehzadalgo browser extension install` to refresh the installed files under your ShehzadAlgo state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

ShehzadAlgo ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want ShehzadAlgo to control.
...
- `ON`: attached; ShehzadAlgo can drive that tab.
...
Debugging: `shehzadalgo sandbox explain`
...
`shehzadalgo browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `shehzadalgo browser extension install` first to copy the extension to a stable location under your ShehzadAlgo state directory.
...
- **This is not isolated** like the dedicated shehzadalgo-managed profile.
