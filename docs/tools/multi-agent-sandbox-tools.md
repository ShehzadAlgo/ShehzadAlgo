...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `shehzadalgo sandbox explain`.
...
Legacy `agent.*` configs are migrated by `shehzadalgo doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=shehzadalgo-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${SHEHZADALGO_STATE_DIR:-$HOME/.shehzadalgo}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
