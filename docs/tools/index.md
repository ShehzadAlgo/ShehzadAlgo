---
summary: "Agent tool surface for ShehzadAlgo (browser, canvas, nodes, message, cron) replacing legacy `shehzadalgo-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `shehzadalgo-*` skills
title: "Tools"
---

# Tools (ShehzadAlgo)

ShehzadAlgo exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `shehzadalgo-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `shehzadalgo.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, ShehzadAlgo logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:shehzadalgo`: all built-in ShehzadAlgo tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`shehzadalgo node run`).
...
ShehzadAlgo tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `shehzadalgo configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated ShehzadAlgo-managed browser.
...
- Quick smoke: `shehzadalgo nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `shehzadalgo gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, ShehzadAlgo clamps `tools.sessions.visibility` to `tree`.
