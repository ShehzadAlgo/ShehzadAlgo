...
- **gateway host** → `shehzadalgo` process on the gateway machine
...
`~/.shehzadalgo/exec-approvals.json` directly.

CLI: `shehzadalgo approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
