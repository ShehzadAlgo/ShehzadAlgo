...
metadata:
  { "shehzadalgo": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---

# tmux Skill (ShehzadAlgo)
...
```bash
SOCKET_DIR="${SHEHZADALGO_TMUX_SOCKET_DIR:-${shehzadalgo_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/shehzadalgo-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/shehzadalgo.sock"
SESSION=shehzadalgo-python
...
- Use `SHEHZADALGO_TMUX_SOCKET_DIR` (legacy `shehzadalgo_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$SHEHZADALGO_TMUX_SOCKET_DIR/shehzadalgo.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `SHEHZADALGO_TMUX_SOCKET_DIR`).
