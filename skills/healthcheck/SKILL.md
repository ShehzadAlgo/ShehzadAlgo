---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for ShehzadAlgo deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, ShehzadAlgo cron scheduling for periodic checks, or version status checks on a machine running ShehzadAlgo (laptop, workstation, Pi, VPS).
---

# ShehzadAlgo Host Hardening

## Overview

Assess and harden the host running ShehzadAlgo, then align it to a user-defined risk tolerance without breaking access. Use ShehzadAlgo security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim ShehzadAlgo changes the host firewall, SSH, or OS updates; it does not.
...
5. ShehzadAlgo gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if ShehzadAlgo can access sensitive data.
...
### 2) Run ShehzadAlgo security audits (read-only)

As part of the default read-only checks, run `shehzadalgo security audit --deep`. Only offer alternatives if the user requests them:

1. `shehzadalgo security audit` (faster, non-probing)
2. `shehzadalgo security audit --json` (structured output)

Offer to apply ShehzadAlgo safe defaults (numbered):

1. `shehzadalgo security audit --fix`

Be explicit that `--fix` only tightens ShehzadAlgo defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check ShehzadAlgo version/update status (read-only)

As part of the default read-only checks, run `shehzadalgo update status`.
...
- Credential hygiene notes (location of ShehzadAlgo creds, prefer disk encryption)
...
- ShehzadAlgo security audit (re-run)
...
After ShehzadAlgo install or first hardening pass, run at least one baseline audit and version check:

- `shehzadalgo security audit`
- `shehzadalgo security audit --deep`
- `shehzadalgo update status`

Ongoing monitoring is recommended. Use the ShehzadAlgo cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `shehzadalgo cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `shehzadalgo update status`
...
Before creating, `shehzadalgo cron list` and match on exact `name`. If found, `shehzadalgo cron edit <id> ...`.
If not found, `shehzadalgo cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `shehzadalgo update status` (preferred for source checkouts and channels)
2. `npm view shehzadalgo version` (published npm version)

## ShehzadAlgo command accuracy

Use only supported commands and flags:

- `shehzadalgo security audit [--deep] [--fix] [--json]`
- `shehzadalgo status` / `shehzadalgo status --deep`
- `shehzadalgo health --json`
- `shehzadalgo update status`
- `shehzadalgo cron add|list|runs|run`

Do not invent CLI flags or imply ShehzadAlgo enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by ShehzadAlgo compaction:
