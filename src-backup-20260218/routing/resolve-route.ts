...
function listAgents(cfg: ShehzadAlgoConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: ShehzadAlgoConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: ShehzadAlgoConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<ShehzadAlgoConfig, EvaluatedBindingsCache>();
