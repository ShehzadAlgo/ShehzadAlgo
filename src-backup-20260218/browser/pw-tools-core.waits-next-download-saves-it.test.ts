...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredShehzadAlgoTmpDir: vi.fn(() => "/tmp/shehzadalgo"),
}));
vi.mock("../infra/tmp-shehzadalgo-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredShehzadAlgoTmpDir.mockReturnValue("/tmp/shehzadalgo");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredShehzadAlgoTmpDir.mockReturnValue("/tmp/shehzadalgo-preferred");
    ...
    const expectedRootedDownloadsDir = path.join(
      path.sep,
      "tmp",
      "shehzadalgo-preferred",
      "downloads",
    );
    const expectedDownloadsTail = `${path.join("tmp", "shehzadalgo-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredShehzadAlgoTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "shehzadalgo-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "shehzadalgo-preferred", "downloads")}${path.sep}`),
    );
  });
