...
import {
  isChromeCdpReady,
  isChromeReachable,
  launchShehzadAlgoChrome,
  resolveShehzadAlgoUserDataDir,
  stopShehzadAlgoChrome,
} from "./chrome.js";
...
function createProfileContext(
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (isExtension) {
      ...
      throw new Error(
        `Chrome extension relay is running, but no tab is connected. Click the ShehzadAlgo Chrome extension icon on a tab to attach it (profile "${profile.name}").`,
      );
    }
    ...
    if (!httpReachable) {
      ...
      const launched = await launchShehzadAlgoChrome(current.resolved, profile);
    ...
    if (!profileState.running) {
      throw new Error(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by shehzadalgo. ` +
    ...
    await stopShehzadAlgoChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchShehzadAlgoChrome(current.resolved, profile);
      ...
      if (profile.driver === "extension") {
        throw new Error(
          `tab not found (no attached Chrome tabs for profile "${profile.name}"). ` +
            "Click the ShehzadAlgo Browser Relay toolbar icon on the tab you want to control (badge ON).",
        );
      }
