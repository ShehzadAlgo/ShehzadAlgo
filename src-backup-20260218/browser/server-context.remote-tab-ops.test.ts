...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const state = makeState("shehzadalgo");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const shehzadalgo = ctx.forProfile("shehzadalgo");

    const opened = await shehzadalgo.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("shehzadalgo")?.lastTargetId).toBe("CREATED");
  });
});
