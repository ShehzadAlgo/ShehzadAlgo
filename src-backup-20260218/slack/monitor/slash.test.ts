...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    const argMenu = harness.actions.get("shehzadalgo_cmdarg");
    if (!argMenu) {
      throw new Error("Missing arg-menu action handler");
    }
    argMenuHandler = argMenu;
    const argMenuOptions = harness.options.get("shehzadalgo_cmdarg");
    if (!argMenuOptions) {
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("shehzadalgo_cmdarg");
  ...
  it("serves filtered options for external_select menus", async () => {
    ...
    expect(blockId).toContain("shehzadalgo_cmdarg_ext:");
