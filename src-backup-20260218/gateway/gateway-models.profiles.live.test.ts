...
import { resolveShehzadAlgoAgentDir } from "../agents/agent-paths.js";
...
import { ensureShehzadAlgoModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, ShehzadAlgoConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.SHEHZADALGO_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.SHEHZADALGO_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.SHEHZADALGO_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: ShehzadAlgoConfig;
  agentDir: string;
}): ShehzadAlgoConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<ShehzadAlgoConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.SHEHZADALGO_AGENT_DIR,
  ...
  const hostAgentDir = resolveShehzadAlgoAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-live-state-"));
  ...
  process.env.SHEHZADALGO_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.shehzadalgo-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveShehzadAlgoAgentDir();
  const sanitizedCfg: ShehzadAlgoConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "ShehzadAlgo live tool probe (local, safe): " +
    ...
    process.env.SHEHZADALGO_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.SHEHZADALGO_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureShehzadAlgoModelsJson(cfg);
      ...
      const rawModels = process.env.SHEHZADALGO_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.shehzadalgo-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
