...
export function resolveHooksConfig(cfg: ShehzadAlgoConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: ShehzadAlgoConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-shehzadalgo-token"] === "string"
      ? req.headers["x-shehzadalgo-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
