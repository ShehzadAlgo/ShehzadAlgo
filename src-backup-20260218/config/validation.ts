...
import type { ShehzadAlgoConfig, ConfigValidationIssue } from "./types.js";
import { ShehzadAlgoSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: ShehzadAlgoConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: ShehzadAlgoConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ShehzadAlgoSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ShehzadAlgoConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ShehzadAlgoConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as ShehzadAlgoConfig,
  };
}
