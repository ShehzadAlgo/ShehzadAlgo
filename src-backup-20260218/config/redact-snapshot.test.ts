...
import type { ConfigFileSnapshot } from "./types.shehzadalgo.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.shehzadalgo/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/shehzadalgo/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/shehzadalgo/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/shehzadalgo/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/shehzadalgo/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = ShehzadAlgoSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "ShehzadAlgoConfig";
    const hints = mapSensitivePaths(ShehzadAlgoSchema, "", {});
