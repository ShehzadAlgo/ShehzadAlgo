...
const ETC_SHEHZADALGO_DIR = path.join(ROOT_DIR, "etc", "shehzadalgo");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "shehzadalgo.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcShehzadAlgoPath(...parts: string[]) {
  return path.join(ETC_SHEHZADALGO_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcShehzadAlgoPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "shehzadalgo.json"))).toEqual({
      shared: true,
    });
  });
});
