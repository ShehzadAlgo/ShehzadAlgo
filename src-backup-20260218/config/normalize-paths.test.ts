  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.shehzadalgo/logs/shehzadalgo.log" },
        hooks: {
          path: "~/.shehzadalgo/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.shehzadalgo/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".shehzadalgo", "logs", "shehzadalgo.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".shehzadalgo", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".shehzadalgo", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".shehzadalgo", "agents", "main"));
