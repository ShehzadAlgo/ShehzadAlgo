...
import {   return { valid: true, warnings: [] };
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "shehzadalgo.sandbox=1");
  args.push("--label", `shehzadalgo.sessionKey=${params.scopeKey}`);
  args.push("--label", `shehzadalgo.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `shehzadalgo.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "shehzadalgo.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`shehzadalgo sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`shehzadalgo sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("shehzadalgo sandbox recreate --all");
}
