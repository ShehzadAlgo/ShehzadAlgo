...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("shehzadalgo-ws-", async (workspaceDir) => {
      await withTempDir("shehzadalgo-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createShehzadAlgoCodingTools({ workspaceDir });
    ...
    await withTempDir("shehzadalgo-ws-", async (workspaceDir) => {
      await withTempDir("shehzadalgo-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello shehzadalgo");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("shehzadalgo-ws-", async (workspaceDir) => {
      await withTempDir("shehzadalgo-override-", async (overrideDir) => {
        const tools = createShehzadAlgoCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("shehzadalgo-sandbox-", async (sandboxDir) => {
      await withTempDir("shehzadalgo-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          containerName: "shehzadalgo-sbx-test",
        ...
        const tools = createShehzadAlgoCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
