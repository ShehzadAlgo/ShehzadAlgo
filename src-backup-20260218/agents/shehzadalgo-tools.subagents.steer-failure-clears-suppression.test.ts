...
} from "./shehzadalgo-tools.subagents.test-harness.js";
import "./test-helpers/fast-core-tools.js";

let createShehzadAlgoTools: (typeof import("./shehzadalgo-tools.js"))["createShehzadAlgoTools"];
...
describe("shehzadalgo-tools: subagents steer failure", () => {
  beforeEach(async () => {
    vi.resetModules();
    ({ createShehzadAlgoTools } = await import("./shehzadalgo-tools.js"));
    ...
    const storePath = path.join(
      os.tmpdir(),
      `shehzadalgo-subagents-steer-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    setSubagentsConfigOverride({
      session: {
        mainKey: "main",
