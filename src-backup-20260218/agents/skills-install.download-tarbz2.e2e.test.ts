...
async function withTempWorkspace(
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-skills-install-"));
  try {
...
function restoreShehzadAlgoStateDir(originalValue: string | undefined): void {
  if (originalValue === undefined) {
    delete process.env.SHEHZADALGO_STATE_DIR;
    return;
  }
  process.env.SHEHZADALGO_STATE_DIR = originalValue;
}

const originalStateDir = process.env.SHEHZADALGO_STATE_DIR;

afterEach(() => {
  restoreShehzadAlgoStateDir(originalStateDir);
});
