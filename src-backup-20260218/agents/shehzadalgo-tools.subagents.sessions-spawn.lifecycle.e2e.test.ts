...
} from "./shehzadalgo-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateShehzadAlgoTools = (typeof import("./shehzadalgo-tools.js"))["createShehzadAlgoTools"];
type CreateShehzadAlgoToolsOpts = Parameters<CreateShehzadAlgoTools>[0];

async function getSessionsSpawnTool(opts: CreateShehzadAlgoToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createShehzadAlgoTools } = await import("./shehzadalgo-tools.js");
  const tool = createShehzadAlgoTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("shehzadalgo-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
