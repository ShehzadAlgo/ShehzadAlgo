...
export function buildServiceEnvironment(params: {
  ...
  const stateDir = env.SHEHZADALGO_STATE_DIR;
  const configPath = env.SHEHZADALGO_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    SHEHZADALGO_PROFILE: profile,
    SHEHZADALGO_STATE_DIR: stateDir,
    SHEHZADALGO_CONFIG_PATH: configPath,
    SHEHZADALGO_GATEWAY_PORT: String(port),
    SHEHZADALGO_GATEWAY_TOKEN: token,
    SHEHZADALGO_LAUNCHD_LABEL: resolvedLaunchdLabel,
    SHEHZADALGO_SYSTEMD_UNIT: systemdUnit,
    SHEHZADALGO_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    SHEHZADALGO_SERVICE_KIND: GATEWAY_SERVICE_KIND,
