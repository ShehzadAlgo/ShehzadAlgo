...
import { POSIX_SHEHZADALGO_TMP_DIR, resolvePreferredShehzadAlgoTmpDir } from "./tmp-shehzadalgo-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredShehzadAlgoTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `shehzadalgo-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredShehzadAlgoTmpDir({
    accessSync,
...
describe("resolvePreferredShehzadAlgoTmpDir", () => {
  it("prefers /tmp/shehzadalgo when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_SHEHZADALGO_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/shehzadalgo when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_SHEHZADALGO_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/shehzadalgo when /tmp/shehzadalgo is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/shehzadalgo when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/shehzadalgo is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/shehzadalgo is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/shehzadalgo is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
