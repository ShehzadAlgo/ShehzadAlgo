...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ShehzadAlgoConfig | null = null;
  ...
  get(key: string, cfg: ShehzadAlgoConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ShehzadAlgoConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: ShehzadAlgoConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: ShehzadAlgoConfig): void {
