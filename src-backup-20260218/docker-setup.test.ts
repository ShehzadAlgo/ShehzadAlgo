...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "shehzadalgo-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    SHEHZADALGO_GATEWAY_TOKEN: "test-token",
    SHEHZADALGO_CONFIG_DIR: join(sandbox.rootDir, "config"),
    SHEHZADALGO_WORKSPACE_DIR: join(sandbox.rootDir, "shehzadalgo"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        SHEHZADALGO_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        SHEHZADALGO_EXTRA_MOUNTS: undefined,
        SHEHZADALGO_HOME_VOLUME: "shehzadalgo-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SHEHZADALGO_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("SHEHZADALGO_EXTRA_MOUNTS=");
    expect(envFile).toContain("SHEHZADALGO_HOME_VOLUME=shehzadalgo-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("shehzadalgo-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("shehzadalgo-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg SHEHZADALGO_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
