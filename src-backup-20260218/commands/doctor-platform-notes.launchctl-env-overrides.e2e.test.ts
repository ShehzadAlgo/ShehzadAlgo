...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SHEHZADALGO_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SHEHZADALGO_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SHEHZADALGO_GATEWAY_TOKEN");
    expect(message).not.toContain("SHEHZADALGO_GATEWAY_PASSWORD");
  });
