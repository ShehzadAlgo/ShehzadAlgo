...
export function listAgentEntries(cfg: ShehzadAlgoConfig): AgentEntry[] {
...
function resolveAgentName(cfg: ShehzadAlgoConfig, agentId: string) {
...
function resolveAgentModel(cfg: ShehzadAlgoConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ShehzadAlgoConfig): AgentSummary[] {
