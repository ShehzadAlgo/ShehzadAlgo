...
beforeAll(() => {
  envSnapshot = captureEnv(["SHEHZADALGO_PROFILE"]);
  process.env.SHEHZADALGO_PROFILE = "isolated";
});
...
vi.mock("../infra/shehzadalgo-root.js", () => ({
  resolveShehzadAlgoPackageRoot: vi.fn().mockResolvedValue("/tmp/shehzadalgo"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/shehzadalgo/pnpm-lock.yaml",
      markerPath: "/tmp/shehzadalgo/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("ShehzadAlgo status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("shehzadalgo status --all") ||
          l.includes("shehzadalgo --profile isolated status --all") ||
          l.includes("shehzadalgo status --all") ||
          l.includes("shehzadalgo --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.SHEHZADALGO_GATEWAY_TOKEN = "abcd1234";
    try {
