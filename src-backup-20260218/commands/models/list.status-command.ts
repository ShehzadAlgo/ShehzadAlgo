...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveShehzadAlgoAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("shehzadalgo models auth setup-token")}\` or \`${formatCliCommand("shehzadalgo configure")}\`.`
          : `Run \`${formatCliCommand("shehzadalgo configure")}\` or set an API key env var.`;
