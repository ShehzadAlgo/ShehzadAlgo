...
function createReplyConfig(home: string, streamMode?: "block"): ShehzadAlgoConfig {
  return {
    agents: {
      defaults: {
...
describe("block streaming", () => {
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-stream-"));
  });
  ...
  beforeEach(() => {
    vi.stubEnv("SHEHZADALGO_TEST_FAST", "1");
