...
This directory contains hooks that ship with ShehzadAlgo. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.shehzadalgo/shehzadalgo.json`:
...
2. Restart gateway: `pkill -9 -f 'shehzadalgo.*gateway' && pnpm shehzadalgo gateway`
3. Enable the hook: `shehzadalgo hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.shehzadalgo.ai/automation/hooks
