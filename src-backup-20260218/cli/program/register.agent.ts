...
${formatHelpExamples([
  ['shehzadalgo agent --to +15555550123 --message "status update"', "Start a new session."],
  ['shehzadalgo agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'shehzadalgo agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'shehzadalgo agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['shehzadalgo agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'shehzadalgo agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.shehzadalgo.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.shehzadalgo.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['shehzadalgo agents set-identity --agent main --name "ShehzadAlgo" --emoji "🦞"', "Set name + emoji."],
  ["shehzadalgo agents set-identity --agent main --avatar avatars/shehzadalgo.png", "Set avatar path."],
  [
    "shehzadalgo agents set-identity --workspace ~/.shehzadalgo/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "shehzadalgo agents set-identity --identity-file ~/.shehzadalgo/workspace/IDENTITY.md --agent main",
