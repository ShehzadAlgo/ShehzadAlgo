...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.SHEHZADALGO_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.SHEHZADALGO_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<ShehzadAlgoConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live ShehzadAlgo docs",
    hasSubcommands: false,
    register: async (program) => {
