...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: ShehzadAlgoConfig; warnings: string[] } {
...
function enablePluginInConfig(config: ShehzadAlgoConfig, pluginId: string): ShehzadAlgoConfig {
  return {
    ...config,
    plugins: {
      ...config.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage ShehzadAlgo plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.shehzadalgo.ai/cli/plugins")}\n`,
    );
    ...
    .action(async (raw: string, opts: { link?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          let next: ShehzadAlgoConfig = enablePluginInConfig(
            {
              ...cfg,
              plugins: {
                ...cfg.plugins,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.shehzadalgo.ai/plugin");
      lines.push("");
