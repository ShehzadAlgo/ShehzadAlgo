...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.shehzadalgo.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + ShehzadAlgo hooks")
    ...
    .option("--hook-url <url>", "ShehzadAlgo hook URL")
    .option("--hook-token <token>", "ShehzadAlgo hook token")
