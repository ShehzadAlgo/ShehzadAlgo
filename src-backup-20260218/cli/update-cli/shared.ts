...
import { resolveShehzadAlgoPackageRoot } from "../../infra/shehzadalgo-root.js";
...
const SHEHZADALGO_REPO_URL = "https://github.com/shehzadalgo/shehzadalgo.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "shehzadalgo";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("shehzadalgo@")) {
    return trimmed.slice("shehzadalgo@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.SHEHZADALGO_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveShehzadAlgoPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", SHEHZADALGO_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `SHEHZADALGO_GIT_DIR points at a non-git directory: ${params.dir}. Set SHEHZADALGO_GIT_DIR to an empty folder or an shehzadalgo checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`SHEHZADALGO_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "shehzadalgo.mjs");
