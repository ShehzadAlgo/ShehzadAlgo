    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "shehzadalgo node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["shehzadalgo node status", "Check node host service status."],
          ["shehzadalgo node install", "Install the node host service."],
          ["shehzadalgo node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.shehzadalgo.ai/cli/node")}\n`,
    );
