...
<title>ShehzadAlgo Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.shehzadalgoCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.shehzadalgoCanvasA2UIAction &&
        typeof window.shehzadalgoCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.shehzadalgoSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("shehzadalgo:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android ShehzadAlgo node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.shehzadalgoSendUserAction === "function"
        ? window.shehzadalgoSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SHEHZADALGO_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.SHEHZADALGO_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>ShehzadAlgo Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
