...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-embedded-agent-"));
...
const makeOpenAiConfig = (modelIds: string[]) =>
  ...
  }) satisfies ShehzadAlgoConfig;

const ensureModels = (cfg: ShehzadAlgoConfig) => ensureShehzadAlgoModelsJson(cfg, agentDir) as unknown;
...
describe("runEmbeddedPiAgent", () => {
  ...
  it("persists prompt transport errors as transcript entries", async () => {
    ...
    const promptErrorEntry = entries.find(
      (entry) => entry.type === "custom" && entry.customType === "shehzadalgo:prompt-error",
