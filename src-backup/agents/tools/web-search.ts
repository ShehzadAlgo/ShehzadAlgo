...
type WebSearchConfig = NonNullable<ShehzadAlgoConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: ShehzadAlgoConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("shehzadalgo configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.shehzadalgo.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://shehzadalgo.ai",
      "X-Title": "ShehzadAlgo Web Search",
    },
