...
function resolveSkillsLimits(config?: ShehzadAlgoConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "shehzadalgo-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "shehzadalgo-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveShehzadAlgoMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: ShehzadAlgoConfig }): {
...
export function buildWorkspaceSkillSnapshot(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`shehzadalgo skills check\` to audit.`
    : "";
