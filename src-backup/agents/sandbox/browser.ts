...
import {
  ...
  DEFAULT_SHEHZADALGO_BROWSER_COLOR,
  DEFAULT_SHEHZADALGO_BROWSER_PROFILE_NAME,
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_SHEHZADALGO_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_SHEHZADALGO_BROWSER_PROFILE_NAME,
    extraArgs: [],
    profiles: {
      [DEFAULT_SHEHZADALGO_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "shehzadalgo.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `shehzadalgo sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `shehzadalgo sandbox recreate --browser --agent ${agentId}`;
          }
          return "shehzadalgo sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: { "shehzadalgo.sandboxBrowser": "1" },
    ...
    args.push("-e", `SHEHZADALGO_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `SHEHZADALGO_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `SHEHZADALGO_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `SHEHZADALGO_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `SHEHZADALGO_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_SHEHZADALGO_BROWSER_PROFILE_NAME)
    : null;
