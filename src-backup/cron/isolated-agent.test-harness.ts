...
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "shehzadalgo-cron-" });
}
...
export async function writeSessionStore(
  ...
  const dir = path.join(home, ".shehzadalgo", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<ShehzadAlgoConfig> = {},
): ShehzadAlgoConfig {
  const base: ShehzadAlgoConfig = {
    agents: {
      defaults: {
