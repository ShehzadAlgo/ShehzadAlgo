...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withSandboxMediaTempHome("shehzadalgo-triggers-", async (home) => {
      const inboundDir = join(home, ".shehzadalgo", "media", "inbound");
      ...
      await stageSandboxMedia({
        ...
        workspaceDir: join(home, "shehzadalgo"),
      });
  ...
  it("rejects staging host files from outside the media directory", async () => {
    await withSandboxMediaTempHome("shehzadalgo-triggers-bypass-", async (home) => {
      // Sensitive host file outside .shehzadalgo
