...
import { scheduleGatewaySigusr1Restart, triggerShehzadAlgoRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting ShehzadAlgo in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerShehzadAlgoRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting ShehzadAlgo via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
