...
import { loadShehzadAlgoPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `shehzadalgo-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.SHEHZADALGO_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "shehzadalgo.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.SHEHZADALGO_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadShehzadAlgoPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.SHEHZADALGO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadShehzadAlgoPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadShehzadAlgoPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadShehzadAlgoPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
