...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ShehzadAlgoPluginDefinition;
  register?: ShehzadAlgoPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ShehzadAlgoPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ShehzadAlgoPluginDefinition;
...
export function loadShehzadAlgoPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverShehzadAlgoPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "shehzadalgo/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "shehzadalgo/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: ShehzadAlgoPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as ShehzadAlgoPluginModule;
    } catch (err) {
