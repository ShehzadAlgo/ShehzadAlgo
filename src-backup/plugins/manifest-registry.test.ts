...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `shehzadalgo-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "shehzadalgo.plugin.json"), JSON.stringify(manifest), "utf-8");
}
