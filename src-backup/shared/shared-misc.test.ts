...
describe("shared/frontmatter", () => {
  ...
  test("resolveShehzadAlgoManifestBlock parses JSON5 metadata and picks shehzadalgo block", () => {
    const frontmatter = {
      metadata: "{ shehzadalgo: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveShehzadAlgoManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveShehzadAlgoManifestBlock returns undefined for invalid input", () => {
    expect(resolveShehzadAlgoManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveShehzadAlgoManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveShehzadAlgoManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
