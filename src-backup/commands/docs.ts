...
const SEARCH_TOOL = "https://docs.shehzadalgo.ai/mcp.SearchShehzadAlgo";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.shehzadalgo.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('shehzadalgo docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.shehzadalgo.ai/");
      runtime.log(`Search: ${formatCliCommand('shehzadalgo docs "your query"')}`);
    }
    return;
  }
