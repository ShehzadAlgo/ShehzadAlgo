...
function stripUnknownConfigKeys(config: ShehzadAlgoConfig): {
  config: ShehzadAlgoConfig;
  removed: string[];
} {
  const parsed = ShehzadAlgoSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: ShehzadAlgoConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: ShehzadAlgoConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: ShehzadAlgoConfig): Promise<{
  config: ShehzadAlgoConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: ShehzadAlgoConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: ShehzadAlgoConfig): {
  config: ShehzadAlgoConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: ShehzadAlgoConfig): {
  config: ShehzadAlgoConfig;
  changes: string[];
} {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".shehzadalgo");
  const targetPath = path.join(targetDir, "shehzadalgo.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".shehzadalgo", "shehzadalgo.json"),
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: ShehzadAlgoConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("shehzadalgo doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("shehzadalgo doctor --fix")}" to apply these changes.`);
    }
  }
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("shehzadalgo doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("shehzadalgo doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("shehzadalgo doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "shehzadalgo doctor --fix" to remove these keys.');
    }
  }
