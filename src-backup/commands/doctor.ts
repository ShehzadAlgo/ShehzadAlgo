...
function resolveMode(cfg: ShehzadAlgoConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("ShehzadAlgo doctor");

  const root = await resolveShehzadAlgoPackageRoot({
  ...
  let cfg: ShehzadAlgoConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("shehzadalgo configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("shehzadalgo config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("shehzadalgo setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("shehzadalgo doctor --fix")}" to apply changes.`);
  }
