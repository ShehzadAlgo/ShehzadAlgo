...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/shehzadalgo" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "shehzadalgo",
        profiles: {
          shehzadalgo: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchShehzadAlgoChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveShehzadAlgoUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopShehzadAlgoChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.SHEHZADALGO_GATEWAY_PORT;
    process.env.SHEHZADALGO_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.SHEHZADALGO_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.SHEHZADALGO_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.SHEHZADALGO_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.SHEHZADALGO_GATEWAY_TOKEN;
    } else {
      process.env.SHEHZADALGO_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.SHEHZADALGO_GATEWAY_PASSWORD;
    } else {
      process.env.SHEHZADALGO_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
