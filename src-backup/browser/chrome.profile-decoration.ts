...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".shehzadalgo-profile-decorated");
}
 ...
 */
export function decorateShehzadAlgoProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SHEHZADALGO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SHEHZADALGO_BROWSER_COLOR).toUpperCase();
