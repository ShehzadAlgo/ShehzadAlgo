...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-shehzadalgo-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-shehzadalgo-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-shehzadalgo-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the ShehzadAlgo gateway (ShehzadAlgo.app menubar, or \`${formatCliCommand("shehzadalgo gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the ShehzadAlgo browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the ShehzadAlgo browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
