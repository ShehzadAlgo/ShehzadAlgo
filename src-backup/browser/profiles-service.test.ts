...
vi.mock("./chrome.js", () => ({
  resolveShehzadAlgoUserDataDir: vi.fn(() => "/tmp/shehzadalgo-test/shehzadalgo/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveShehzadAlgoUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "shehzadalgo-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveShehzadAlgoUserDataDir).mockReturnValue(userDataDir);
