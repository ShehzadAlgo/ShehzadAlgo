...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/shehzadalgo/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (ShehzadAlgo)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.SHEHZADALGO_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "shehzadalgo");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.SHEHZADALGO_TAILNET_DNS?.trim();
