    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "shehzadalgo", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-shehzadalgo-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-shehzadalgo-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "shehzadalgo", messages: [{ role: "user", content: "hi" }] },
          {
            "x-shehzadalgo-agent-id": "beta",
            "x-shehzadalgo-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
