...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.SHEHZADALGO_STATE_DIR;
  previousConfigPath = process.env.SHEHZADALGO_CONFIG_PATH;
  previousSkipBrowserControl = process.env.SHEHZADALGO_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.SHEHZADALGO_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.SHEHZADALGO_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.SHEHZADALGO_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.SHEHZADALGO_SKIP_CHANNELS;
  previousSkipProviders = process.env.SHEHZADALGO_SKIP_PROVIDERS;
  previousSkipCron = process.env.SHEHZADALGO_SKIP_CRON;
  previousMinimalGateway = process.env.SHEHZADALGO_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.SHEHZADALGO_STATE_DIR = path.join(tempHome, ".shehzadalgo");
...
function applyGatewaySkipEnv() {
  ...
  process.env.SHEHZADALGO_SKIP_PROVIDERS = "1";
  process.env.SHEHZADALGO_SKIP_CRON = "1";
  process.env.SHEHZADALGO_TEST_MINIMAL_GATEWAY = "1";
  process.env.SHEHZADALGO_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "shehzadalgo-test-no-bundled-extensions")
    : "shehzadalgo-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "shehzadalgo-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".shehzadalgo-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.SHEHZADALGO_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.SHEHZADALGO_CONFIG_PATH;
    } else {
      process.env.SHEHZADALGO_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.SHEHZADALGO_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.SHEHZADALGO_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.SHEHZADALGO_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SHEHZADALGO_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.SHEHZADALGO_SKIP_CANVAS_HOST;
    } else {
      process.env.SHEHZADALGO_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.SHEHZADALGO_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.SHEHZADALGO_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.SHEHZADALGO_SKIP_CHANNELS;
    } else {
      process.env.SHEHZADALGO_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.SHEHZADALGO_SKIP_PROVIDERS;
    } else {
      process.env.SHEHZADALGO_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.SHEHZADALGO_SKIP_CRON;
    } else {
      process.env.SHEHZADALGO_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.SHEHZADALGO_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.SHEHZADALGO_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["SHEHZADALGO_GATEWAY_TOKEN"]);
  const prev = process.env.SHEHZADALGO_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SHEHZADALGO_GATEWAY_TOKEN = fallbackToken;
  }
