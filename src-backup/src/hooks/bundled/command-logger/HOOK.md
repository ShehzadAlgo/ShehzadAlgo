...
homepage: https://docs.shehzadalgo.ai/automation/hooks#command-logger
metadata:
  {
    "shehzadalgo":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.shehzadalgo/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.shehzadalgo/logs/commands.log ~/.shehzadalgo/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/shehzadalgo`:
   ```
   /home/username/.shehzadalgo/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
