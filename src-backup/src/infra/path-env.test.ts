...
let ensureShehzadAlgoCliOnPath: typeof import("./path-env.js").ensureShehzadAlgoCliOnPath;

describe("ensureShehzadAlgoCliOnPath", () => {
  const envKeys = [
    "PATH",
    "SHEHZADALGO_PATH_BOOTSTRAPPED",
    "SHEHZADALGO_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureShehzadAlgoCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling shehzadalgo exists", () => {
    const tmp = abs("/tmp/shehzadalgo-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "shehzadalgo");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.SHEHZADALGO_PATH_BOOTSTRAPPED;

    ensureShehzadAlgoCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.SHEHZADALGO_PATH_BOOTSTRAPPED = "1";
    ensureShehzadAlgoCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/shehzadalgo-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "shehzadalgo");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/shehzadalgo-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "shehzadalgo");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "shehzadalgo");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/shehzadalgo-path/case-linuxbrew");
