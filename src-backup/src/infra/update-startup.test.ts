...
describe("update-startup", () => {
  ...
  let resolveShehzadAlgoPackageRoot: (typeof import("./shehzadalgo-root.js"))["resolveShehzadAlgoPackageRoot"];
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "SHEHZADALGO_STATE_DIR");
    prevStateDir = process.env.SHEHZADALGO_STATE_DIR;
    process.env.SHEHZADALGO_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveShehzadAlgoPackageRoot } = await import("./shehzadalgo-root.js"));
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    vi.mocked(resolveShehzadAlgoPackageRoot).mockResolvedValue("/opt/shehzadalgo");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/shehzadalgo",
      installKind: "package",
      packageManager: "npm",
