...
import { resolveShehzadAlgoPackageRoot, resolveShehzadAlgoPackageRootSync } from "./shehzadalgo-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveShehzadAlgoPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "shehzadalgo" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveShehzadAlgoPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // shehzadalgo.mjs or dist/<bundle>.js
