...
vi.mock("./shehzadalgo-root.js", () => ({
  resolveShehzadAlgoPackageRoot: vi.fn(async () => null),
  resolveShehzadAlgoPackageRootSync: vi.fn(() => null),
}));
...
describe("control UI assets helpers (fs-mocked)", () => {
  ...
  it("uses resolveShehzadAlgoPackageRoot when available", async () => {
    const shehzadalgoRoot = await import("./shehzadalgo-root.js");
    const { resolveControlUiDistIndexPath } = await import("./control-ui-assets.js");

    const pkgRoot = abs("fixtures/shehzadalgo");
    (
      shehzadalgoRoot.resolveShehzadAlgoPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/shehzadalgo"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "shehzadalgo" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "shehzadalgo.mjs"))).resolves.toBe(
  ...
  it("returns null when fallback package name does not match", async () => {
    ...
    const root = abs("fixtures/not-shehzadalgo");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    ...
    const pkgRoot = abs("fixtures/shehzadalgo-bundle");
    (
      shehzadalgoRoot.resolveShehzadAlgoPackageRootSync as unknown as ReturnType<typeof vi.fn>
