...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SHEHZADALGO_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*shehzadalgo-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "shehzadalgo";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "shehzadalgo");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "shehzadalgo-gw");
  ...
  records.push(`_shehzadalgo-gw._tcp IN PTR ${instanceLabel}._shehzadalgo-gw._tcp`);
  records.push(`${instanceLabel}._shehzadalgo-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._shehzadalgo-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; shehzadalgo-content-hash: ${contentHash}\n${contentBody}`;
}
