...
type EnsureShehzadAlgoPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureShehzadAlgoPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `shehzadalgo` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "shehzadalgo");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.SHEHZADALGO_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "shehzadalgo"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `shehzadalgo` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureShehzadAlgoCliOnPath(opts: EnsureShehzadAlgoPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SHEHZADALGO_PATH_BOOTSTRAPPED)) {
    return;
  }
