...
function summarizeGroupPolicy(cfg: ShehzadAlgoConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: ShehzadAlgoConfig): boolean {
...
function collectModels(cfg: ShehzadAlgoConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: ShehzadAlgoConfig): Set<string> {
  const baseCfg: ShehzadAlgoConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: ShehzadAlgoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: ShehzadAlgoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: ShehzadAlgoConfig): boolean {
...
function isBrowserEnabled(cfg: ShehzadAlgoConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: ShehzadAlgoConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: ShehzadAlgoConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep SHEHZADALGO_STATE_DIR on a local-only volume and re-run "${formatCliCommand("shehzadalgo security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: ShehzadAlgoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer SHEHZADALGO_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const shehzadalgoGatewayToken =
    typeof env.SHEHZADALGO_GATEWAY_TOKEN === "string" && env.SHEHZADALGO_GATEWAY_TOKEN.trim()
      ? env.SHEHZADALGO_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-shehzadalgo-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: ShehzadAlgoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: ShehzadAlgoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: ShehzadAlgoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: ShehzadAlgoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: ShehzadAlgoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: ShehzadAlgoConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
