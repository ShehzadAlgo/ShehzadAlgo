    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["shehzadalgo memory status", "Show index and provider status."],
          ["shehzadalgo memory index --force", "Force a full reindex."],
          ['shehzadalgo memory search --query "deployment notes"', "Search indexed memory entries."],
          ["shehzadalgo memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.shehzadalgo.ai/cli/memory")}\n`,
    );
