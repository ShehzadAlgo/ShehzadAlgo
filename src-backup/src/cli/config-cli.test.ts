...
import type { ConfigFileSnapshot, ShehzadAlgoConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `shehzadalgo config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<(cfg: ShehzadAlgoConfig) => Promise<void>>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: ShehzadAlgoConfig) => mockWriteConfigFile(cfg),
}));
...
function setSnapshot(resolved: ShehzadAlgoConfig, config: ShehzadAlgoConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: ShehzadAlgoConfig = {
        agents: {
      ...
      const runtimeMerged: ShehzadAlgoConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
