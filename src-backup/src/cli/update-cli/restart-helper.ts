...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = env.SHEHZADALGO_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.SHEHZADALGO_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = env.SHEHZADALGO_LAUNCHD_LABEL?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.SHEHZADALGO_PROFILE);
}

function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  const override = env.SHEHZADALGO_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.SHEHZADALGO_PROFILE);
}
...
export async function prepareRestartScript(
  ...
  try {
    if (platform === "linux") {
      ...
      filename = `shehzadalgo-restart-${timestamp}.sh`;
...
`;
      ...
      filename = `shehzadalgo-restart-${timestamp}.bat`;
