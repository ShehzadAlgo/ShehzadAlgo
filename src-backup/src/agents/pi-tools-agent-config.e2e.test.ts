...
import { createShehzadAlgoCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-pi-tools-"));
    ...
    try {
      ...
      const tools = createShehzadAlgoCodingTools({
        config: cfg,
...
*** End Patch`;
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createShehzadAlgoCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createShehzadAlgoCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createShehzadAlgoCodingTools({
      config: cfg,
    ...
    const defaultTools = createShehzadAlgoCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createShehzadAlgoCodingTools({
      config: cfg,
    ...
    const bobTools = createShehzadAlgoCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createShehzadAlgoCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createShehzadAlgoCodingTools({
      config: cfg,
