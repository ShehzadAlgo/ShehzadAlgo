...
function resolveExecConfig(params: { cfg?: ShehzadAlgoConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: ShehzadAlgoConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createShehzadAlgoCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createShehzadAlgoReadTool(freshReadTool);
