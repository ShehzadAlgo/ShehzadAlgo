...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses SHEHZADALGO_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SHEHZADALGO_HOME: "/srv/shehzadalgo-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/shehzadalgo-home"), ".shehzadalgo", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".shehzadalgo", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("shehzadalgo-workspace-");
