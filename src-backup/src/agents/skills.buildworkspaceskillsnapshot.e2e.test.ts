...
describe("buildWorkspaceSkillSnapshot", () => {
  ...
  it("limits discovery for nested repo-style skills roots (dir/skills/*)", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-"));
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-skills-repo-"));
  ...
  it("enforces maxSkillFileBytes for root-level SKILL.md", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-"));
    const rootSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-root-skill-"));
