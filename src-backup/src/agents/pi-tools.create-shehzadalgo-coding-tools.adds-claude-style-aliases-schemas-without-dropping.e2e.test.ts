...
import { createShehzadAlgoTools } from "./shehzadalgo-tools.js";
import { __testing, createShehzadAlgoCodingTools } from "./pi-tools.js";
...
describe("createShehzadAlgoCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createShehzadAlgoTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createShehzadAlgoCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "shehzadalgo-outside.txt");
