...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses SHEHZADALGO_HOME for tilde expansion", () => {
    vi.stubEnv("SHEHZADALGO_HOME", "/srv/shehzadalgo-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/shehzadalgo-home", "cron", "jobs.json"));
  });
});
