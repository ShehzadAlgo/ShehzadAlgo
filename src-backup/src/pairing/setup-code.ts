...
function resolveGatewayPort(cfg: ShehzadAlgoConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: ShehzadAlgoConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.SHEHZADALGO_GATEWAY_PASSWORD?.trim() ||
    env.SHEHZADALGO_GATEWAY_PASSWORD?.trim() ||
