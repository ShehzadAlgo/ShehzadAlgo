...
function cfg(accounts?: Record<string, unknown> | null): ShehzadAlgoConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as ShehzadAlgoConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as ShehzadAlgoConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as ShehzadAlgoConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as ShehzadAlgoConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as ShehzadAlgoConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as ShehzadAlgoConfig)).toBe("default");
    });
  });
});
