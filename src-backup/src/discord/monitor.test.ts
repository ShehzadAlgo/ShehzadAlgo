...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of ShehzadAlgo")).toBe("friends-of-shehzadalgo");
  ...
  it("matches ids or names", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "steipete", "Friends of ShehzadAlgo"],
    ...
    expect(allowListMatches(allow, { name: "friends-of-shehzadalgo" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-shehzadalgo" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of ShehzadAlgo"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-shehzadalgo");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-shehzadalgo": { slug: "friends-of-shehzadalgo" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of ShehzadAlgo"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["shehzadalgo-dm"],
        channelId: "1",
        channelName: "ShehzadAlgo DM",
        channelSlug: "shehzadalgo-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
