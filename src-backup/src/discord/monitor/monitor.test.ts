...
describe("agent components", () => {
  const createCfg = (): ShehzadAlgoConfig => ({}) as ShehzadAlgoConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): ShehzadAlgoConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockReset().mockReturnValue("/tmp/shehzadalgo-sessions-test.json");
  });
...
describe("resolveDiscordPresenceUpdate", () => {
  ...
  it("includes streaming url when activityType is streaming", () => {
    ...
    expect(presence?.activities[0]).toMatchObject({
      type: 1,
      name: "Live",
      url: "https://twitch.tv/shehzadalgo",
    });
  });
});
