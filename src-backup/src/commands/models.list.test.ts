...
const ensureShehzadAlgoModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/shehzadalgo-agent/auth.json" });
const resolveShehzadAlgoAgentDir = vi.fn().mockReturnValue("/tmp/shehzadalgo-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/shehzadalgo-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureShehzadAlgoModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/shehzadalgo-agent");
  });
