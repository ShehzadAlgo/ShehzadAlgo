...
export async function updateConfig(
  mutator: (cfg: ShehzadAlgoConfig) => ShehzadAlgoConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: ShehzadAlgoConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ShehzadAlgoConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("shehzadalgo agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
