...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveShehzadAlgoAgentDir: vi.fn().mockReturnValue("/tmp/shehzadalgo-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/shehzadalgo-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/shehzadalgo-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveShehzadAlgoAgentDir: mocks.resolveShehzadAlgoAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/shehzadalgo-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveShehzadAlgoAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/shehzadalgo-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/shehzadalgo-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/shehzadalgo-agent-custom");
