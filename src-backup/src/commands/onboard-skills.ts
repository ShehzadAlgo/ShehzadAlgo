...
export async function setupSkills(
  ...
  let next: ShehzadAlgoConfig = cfg;
  if (installable.length > 0) {
    ...
    for (const name of selected) {
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("shehzadalgo doctor")}\` to review skills + requirements.`,
      );
      runtime.log("Docs: https://docs.shehzadalgo.ai/skills");
    }
  }
