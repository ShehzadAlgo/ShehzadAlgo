...
describe("gateway auth", () => {
  it("resolves token/password from SHEHZADALGO gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          SHEHZADALGO_GATEWAY_TOKEN: "env-token",
          SHEHZADALGO_GATEWAY_PASSWORD: "env-password",
  ...
  it("does not resolve legacy SHEHZADALGO gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          SHEHZADALGO_GATEWAY_TOKEN: "legacy-token",
          SHEHZADALGO_GATEWAY_PASSWORD: "legacy-password",
        } as NodeJS.ProcessEnv,
      }),
    ).toMatchObject({
      mode: "none",
