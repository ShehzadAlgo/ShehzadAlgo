...
export function isCommandEnabled(cfg: ShehzadAlgoConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ShehzadAlgoConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as ShehzadAlgoConfig),
...
export function getCommandDetection(_cfg?: ShehzadAlgoConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ShehzadAlgoConfig) {
