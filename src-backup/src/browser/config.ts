import type { BrowserConfig, BrowserProfileConfig, ShehzadAlgoConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_SHEHZADALGO_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "shehzadalgo" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_SHEHZADALGO_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "shehzadalgo" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SHEHZADALGO_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SHEHZADALGO_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an ShehzadAlgo browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for shehzadalgo/shehzadalgo CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: ShehzadAlgoConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SHEHZADALGO_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "shehzadalgo";
