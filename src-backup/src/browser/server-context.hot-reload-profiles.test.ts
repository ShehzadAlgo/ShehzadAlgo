...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", async () => {
    ...
    // Start with only shehzadalgo profile
    ...
    // 2. Simulate adding a new profile to config (like user editing shehzadalgo.json)
  ...
  it("forProfile refreshes existing profile config after loadConfig cache updates", async () => {
    ...
    cfgProfiles.shehzadalgo = { cdpPort: 19999, color: "#FF4500" };
    cachedConfig = null;
    ...
    expect(state.resolved.profiles.shehzadalgo?.cdpPort).toBe(19999);
  });
