...
declare const __SHEHZADALGO_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __SHEHZADALGO_VERSION__ === "string" && __SHEHZADALGO_VERSION__) ||
  process.env.SHEHZADALGO_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `shehzadalgo --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.SHEHZADALGO_GATEWAY_PORT ??
    process.env.SHEHZADALGO_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.SHEHZADALGO_GATEWAY_BIND ??
    process.env.SHEHZADALGO_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (SHEHZADALGO_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
...
void main().catch((err) => {
  console.error(
    "[shehzadalgo] Gateway daemon failed:",
