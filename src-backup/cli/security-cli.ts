...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["shehzadalgo security audit", "Run a local security audit."],
          ["shehzadalgo security audit --deep", "Include best-effort live Gateway probe checks."],
          ["shehzadalgo security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["shehzadalgo security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.shehzadalgo.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("ShehzadAlgo security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("shehzadalgo security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("shehzadalgo security audit --fix")}`));
        if (!fixResult) {
