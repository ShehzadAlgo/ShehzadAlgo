    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["shehzadalgo nodes status", "List known nodes with live status."],
          ["shehzadalgo nodes pairing pending", "Show pending node pairing requests."],
          ['shehzadalgo nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["shehzadalgo nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.shehzadalgo.ai/cli/nodes")}\n`,
    );
