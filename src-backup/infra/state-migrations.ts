  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set SHEHZADALGO_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function autoMigrateLegacyState(params: {
  ...
  if (env.SHEHZADALGO_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
    return {
