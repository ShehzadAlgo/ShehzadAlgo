...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SHEHZADALGO_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "ShehzadAlgo";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(ShehzadAlgo\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.SHEHZADALGO_MDNS_HOSTNAME?.trim() ||
    process.env.SHEHZADALGO_MDNS_HOSTNAME?.trim() ||
    "shehzadalgo";
  const hostname =
    hostnameRaw
