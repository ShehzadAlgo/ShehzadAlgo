...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: ShehzadAlgoConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: ShehzadAlgoConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: ShehzadAlgoConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: ShehzadAlgoConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: ShehzadAlgoConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: ShehzadAlgoConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: ShehzadAlgoConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
