# ShehzadAlgo ACP Bridge

This document describes how the ShehzadAlgo ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`shehzadalgo acp` exposes an ACP agent over stdio and forwards prompts to a running
ShehzadAlgo Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a ShehzadAlgo Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `shehzadalgo acp` over stdio.

Example config:

```bash
shehzadalgo config set gateway.remote.url wss://gateway-host:18789
shehzadalgo config set gateway.remote.token <token>
```

Example run:

```bash
shehzadalgo acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
shehzadalgo acp --session agent:main:main
shehzadalgo acp --session agent:design:main
shehzadalgo acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “ShehzadAlgo ACP” to start a thread.

## Execution Model

- ACP client spawns `shehzadalgo acp` and speaks ACP messages over stdio.
...
```bash
shehzadalgo acp --session agent:main:main
shehzadalgo acp --session-label "support inbox"
shehzadalgo acp --reset-session
```
...
`shehzadalgo acp` resolves the Gateway URL and auth from CLI flags or config:
