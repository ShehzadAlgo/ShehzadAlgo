...
import type { ShehzadAlgoPluginApi, ShehzadAlgoPluginToolContext } from "../../../src/plugins/types.js";
...
function fakeApi(overrides: Partial<ShehzadAlgoPluginApi> = {}): ShehzadAlgoPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<ShehzadAlgoPluginToolContext> = {}): ShehzadAlgoPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-lobster-plugin-"));
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: ShehzadAlgoPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
