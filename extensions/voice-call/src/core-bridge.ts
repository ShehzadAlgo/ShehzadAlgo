...
function resolveShehzadAlgoRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.shehzadalgo_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set shehzadalgo_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveShehzadAlgoRoot(), "dist", "extensionAPI.js");
