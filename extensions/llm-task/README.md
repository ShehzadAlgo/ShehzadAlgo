...
`shehzadalgo.invoke --each`) without adding new shehzadalgo code per workflow.
...
This extension depends on shehzadalgo internal modules (the embedded agent runner).
It is intended to ship as a **bundled** shehzadalgo extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.shehzadalgo/extensions` as a standalone plugin directory.
