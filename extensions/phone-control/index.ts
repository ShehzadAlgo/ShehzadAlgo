...
import type { ShehzadAlgoPluginApi, ShehzadAlgoPluginService } from "shehzadalgo/plugin-sdk";
...
function normalizeDenyList(cfg: ShehzadAlgoPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: ShehzadAlgoPluginApi["config"]): string[] {
...
export default function register(api: ShehzadAlgoPluginApi) {
  ...
  const timerService: ShehzadAlgoPluginService = {
