...
metadata: { "shehzadalgo": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == shehzadalgo `sessions_spawn`
- **File I/O** == shehzadalgo `read`/`write`
- **Remote fetch** == shehzadalgo `web_fetch` (or `exec` with curl when POST is required)
