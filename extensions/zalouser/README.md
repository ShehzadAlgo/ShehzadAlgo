...
shehzadalgo extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
shehzadalgo channels login --channel zalouser
...
```bash
shehzadalgo message send --channel zalouser --target <threadId> --message "Hello from shehzadalgo!"
```
...
```bash
shehzadalgo channels login --channel zalouser              # Login via QR
shehzadalgo channels login --channel zalouser --account work
shehzadalgo channels status --probe
shehzadalgo channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
shehzadalgo directory self --channel zalouser
shehzadalgo directory peers list --channel zalouser --query "name"
shehzadalgo directory groups list --channel zalouser --query "work"
shehzadalgo directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
shehzadalgo message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
shehzadalgo message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `shehzadalgo channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
shehzadalgo channels login --channel zalouser --account work
shehzadalgo message send --channel zalouser --account work --target <id> --message "Hello"
