/**
 * Twitch channel plugin for ShehzadAlgo.
 *
...
import { buildChannelConfigSchema } from "shehzadalgo/plugin-sdk";
 ...
 * for ShehzadAlgo. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: ShehzadAlgoConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: ShehzadAlgoConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: ShehzadAlgoConfig): boolean => {
