...
import { approveDevicePairing, listDevicePairing } from "shehzadalgo/plugin-sdk";
...
function resolveGatewayPort(cfg: ShehzadAlgoPluginApi["config"]): number {
  const envRaw =
    process.env.shehzadalgo_GATEWAY_PORT?.trim() || process.env.shehzadalgo_GATEWAY_PORT?.trim();
  if (envRaw) {
...
function resolveScheme(
  cfg: ShehzadAlgoPluginApi["config"],
...
async function resolveTailnetHost(api: ShehzadAlgoPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: ShehzadAlgoPluginApi["config"]): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    process.env.shehzadalgo_GATEWAY_TOKEN?.trim() ||
    process.env.shehzadalgo_GATEWAY_TOKEN?.trim() ||
    cfg.gateway?.auth?.token?.trim();
  const password =
    process.env.shehzadalgo_GATEWAY_PASSWORD?.trim() ||
    process.env.shehzadalgo_GATEWAY_PASSWORD?.trim() ||
...
async function resolveGatewayUrl(api: ShehzadAlgoPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the ShehzadAlgo iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the ShehzadAlgo iOS app:",
            "",
            "```",
            qrAscii,
            "```",
