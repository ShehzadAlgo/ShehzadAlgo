...
type EnvSnapshot = {
  ...
  shehzadalgoHome: string | undefined;
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    shehzadalgoHome: process.env.SHEHZADALGO_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SHEHZADALGO_HOME", snapshot.shehzadalgoHome);
  restoreKey("SHEHZADALGO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked SHEHZADALGO_HOME.
  delete process.env.SHEHZADALGO_HOME;
  process.env.SHEHZADALGO_STATE_DIR = path.join(base, ".shehzadalgo");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "shehzadalgo-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".shehzadalgo", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
