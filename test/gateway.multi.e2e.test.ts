...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `shehzadalgo-e2e-${name}-`));
  const configDir = path.join(homeDir, ".shehzadalgo");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          SHEHZADALGO_SKIP_BROWSER_CONTROL_SERVER: "1",
    ...
    async () => {
      ...
      const [hookResA, hookResB] = await Promise.all([
        postJson(
          ...
          { "x-shehzadalgo-token": gwA.hookToken },
        ),
        postJson(
          ...
          { "x-shehzadalgo-token": gwB.hookToken },
        ),
      ]);
