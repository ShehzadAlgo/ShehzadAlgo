...
process.env.SHEHZADALGO_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
// Set HOME/state isolation before importing any runtime ShehzadAlgo modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: ShehzadAlgoConfig) => {
    ...
    resolveAccount: (cfg: ShehzadAlgoConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: ShehzadAlgoConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
