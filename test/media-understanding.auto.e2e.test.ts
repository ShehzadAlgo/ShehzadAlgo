...
import type { ShehzadAlgoConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("shehzadalgo-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("shehzadalgo-bin-sherpa-");
      const modelDir = await makeTempDir("shehzadalgo-sherpa-model-");
      ...
      const cfg: ShehzadAlgoConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("shehzadalgo-bin-whispercpp-");
      const modelDir = await makeTempDir("shehzadalgo-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("shehzadalgo-bin-gemini-");
      ...
      const cfg: ShehzadAlgoConfig = { tools: { media: { image: {} } } };
