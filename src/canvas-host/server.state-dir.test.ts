...
describe("canvas host state dir defaults", () => {
  ...
  it("uses shehzadalgo_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("ShehzadAlgo Canvas");
    } finally {
      await handler.close();
