...
import type { ShehzadAlgoConfig } from "../config/types.js";
import { resolvePreferredShehzadAlgoTmpDir } from "../infra/tmp-shehzadalgo-dir.js";
import { readLoggingConfig } from "./config.js";
import type { ConsoleStyle } from "./console.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredShehzadAlgoTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "shehzadalgo.log"); // legacy single-file path

const LOG_PREFIX = "shehzadalgo";
...
function resolveSettings(): ResolvedSettings {
  let cfg: ShehzadAlgoConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => ShehzadAlgoConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.shehzadalgo_TEST_FILE_LOG !== "1" ? "silent" : "info";
