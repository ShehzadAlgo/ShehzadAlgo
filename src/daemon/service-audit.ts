...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.shehzadalgo_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service shehzadalgo_GATEWAY_TOKEN does not match gateway.auth.token in shehzadalgo.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `shehzadalgo gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
