...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.shehzadalgo.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", shehzadalgo_PROFILE: "default" },
    });
...
describe("launchd bootstrap repair", () => {
  it("bootstraps and kickstarts the resolved label", async () => {
    ...
    const label = "ai.shehzadalgo.gateway";
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when shehzadalgo_PROFILE is unset", () => {
    const env = { HOME: "/Users/test" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.shehzadalgo.gateway.plist",
    );
  });

  it("uses profile-specific label when shehzadalgo_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", shehzadalgo_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.shehzadalgo.jbphoenix.plist",
    );
  });

  it("prefers shehzadalgo_LAUNCHD_LABEL over shehzadalgo_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      shehzadalgo_PROFILE: "jbphoenix",
      shehzadalgo_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from shehzadalgo_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      shehzadalgo_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty shehzadalgo_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      shehzadalgo_PROFILE: "myprofile",
      shehzadalgo_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.shehzadalgo.myprofile.plist",
    );
  });
});
