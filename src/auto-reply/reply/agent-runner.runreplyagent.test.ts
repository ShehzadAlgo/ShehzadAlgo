...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { shehzadalgo_STATE_DIR: process.env.shehzadalgo_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.shehzadalgo_STATE_DIR === undefined) {
      delete process.env.shehzadalgo_STATE_DIR;
    } else {
      process.env.shehzadalgo_STATE_DIR = snapshot.shehzadalgo_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "shehzadalgo-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "shehzadalgo-memory-flush-"));
  });
