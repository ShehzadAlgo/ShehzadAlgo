...
describe("media store", () => {
  ...
  beforeAll(async () => {
    ...
    home = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-test-home-"));
    ...
    await fs.mkdir(path.join(home, ".shehzadalgo"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.shehzadalgo${path.sep}media`);
