...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.shehzadalgo_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for ShehzadAlgo. You can keep running the web gateway without it: \`${formatCliCommand("shehzadalgo gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
