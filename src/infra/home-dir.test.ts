...
describe("resolveEffectiveHomeDir", () => {
  it("prefers shehzadalgo_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/shehzadalgo-home"),
    );
  });
  ...
  it("expands shehzadalgo_HOME when set to ~", () => {
    const env = {
      shehzadalgo_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for shehzadalgo_HOME", () => {
    const result = resolveRequiredHomeDir(
      { shehzadalgo_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when shehzadalgo_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ shehzadalgo_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { shehzadalgo_HOME: "/srv/shehzadalgo-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/shehzadalgo-home")}/x`);
  });
