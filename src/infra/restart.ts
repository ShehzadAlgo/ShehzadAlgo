...
export function triggerShehzadAlgoRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.shehzadalgo_SYSTEMD_UNIT,
        process.env.shehzadalgo_PROFILE,
      );
  ...
  const label =
    process.env.shehzadalgo_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.shehzadalgo_PROFILE);
