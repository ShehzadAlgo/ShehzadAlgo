...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.shehzadalgo_MDNS_HOSTNAME = "test-host";
    process.env.shehzadalgo_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("shehzadalgo-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("shehzadalgo (ShehzadAlgo)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("shehzadalgo");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("shehzadalgo.local");
