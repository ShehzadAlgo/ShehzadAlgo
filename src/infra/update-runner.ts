...
const DEFAULT_PACKAGE_NAME = "shehzadalgo";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "shehzadalgo.mjs");
    ...
    const doctorStep = await runStep(
      step("shehzadalgo doctor", doctorArgv, gitRoot, { shehzadalgo_UPDATE_IN_PROGRESS: "1" }),
    );
