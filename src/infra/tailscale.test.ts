...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.shehzadalgo_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.shehzadalgo_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.shehzadalgo_TEST_TAILSCALE_BINARY;
    } else {
      process.env.shehzadalgo_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
