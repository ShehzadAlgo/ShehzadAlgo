...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "shehzadalgo-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "shehzadalgo-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "shehzadalgo-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.shehzadalgo when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers shehzadalgo_HOME over HOME", () => {
    vi.stubEnv("shehzadalgo_HOME", "/srv/shehzadalgo-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/shehzadalgo-home"));
...
describe("shortenHomePath", () => {
  it("uses $shehzadalgo_HOME prefix when shehzadalgo_HOME is set", () => {
    vi.stubEnv("shehzadalgo_HOME", "/srv/shehzadalgo-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/shehzadalgo-home")}/.shehzadalgo/shehzadalgo.json`)).toBe(
      "$shehzadalgo_HOME/.shehzadalgo/shehzadalgo.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $shehzadalgo_HOME replacement when shehzadalgo_HOME is set", () => {
    vi.stubEnv("shehzadalgo_HOME", "/srv/shehzadalgo-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/shehzadalgo-home")}/.shehzadalgo/shehzadalgo.json`),
    ).toBe("config: $shehzadalgo_HOME/.shehzadalgo/shehzadalgo.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/shehzadalgo")).toBe(path.resolve(os.homedir(), "shehzadalgo"));
  });
  ...
  it("prefers shehzadalgo_HOME for tilde expansion", () => {
    vi.stubEnv("shehzadalgo_HOME", "/srv/shehzadalgo-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/shehzadalgo")).toBe(path.resolve("/srv/shehzadalgo-home", "shehzadalgo"));
