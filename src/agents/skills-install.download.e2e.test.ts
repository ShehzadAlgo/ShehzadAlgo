...
const originalShehzadAlgoStateDir = process.env.shehzadalgo_STATE_DIR;

afterEach(() => {
  if (originalShehzadAlgoStateDir === undefined) {
    delete process.env.shehzadalgo_STATE_DIR;
  } else {
    process.env.shehzadalgo_STATE_DIR = originalShehzadAlgoStateDir;
  }
});
