...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["shehzadalgo_STATE_DIR"]);
  let tempStateDir: string | null = null;

  const writePersistedRegistry = async (persisted: Record<string, unknown>) => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "shehzadalgo-subagent-"));
  ...
  it("uses isolated temp state when shehzadalgo_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "shehzadalgo-test-state"));
  });
});
