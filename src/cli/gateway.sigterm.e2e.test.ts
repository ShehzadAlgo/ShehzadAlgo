...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "shehzadalgo-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      shehzadalgo_NO_RESPAWN: "1",
      shehzadalgo_STATE_DIR: stateDir,
      shehzadalgo_SKIP_CHANNELS: "1",
      shehzadalgo_SKIP_GMAIL_WATCHER: "1",
      shehzadalgo_SKIP_CRON: "1",
      shehzadalgo_SKIP_BROWSER_CONTROL_SERVER: "1",
      shehzadalgo_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "shehzadalgo-entry-bootstrap.mjs");
