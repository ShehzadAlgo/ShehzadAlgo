...
export function requireShehzadAlgoAgentDir(): string {
  const agentDir = process.env.shehzadalgo_AGENT_DIR;
  if (!agentDir) {
    throw new Error("shehzadalgo_AGENT_DIR not set");
  }
  return agentDir;
}
