...
function resolveGatewayAuthToken(cfg: ShehzadAlgoConfig, env: NodeJS.ProcessEnv): string | undefined {
  const configToken = cfg.gateway?.auth?.token?.trim();
  if (configToken) {
    return configToken;
  }
  const envToken = env.shehzadalgo_GATEWAY_TOKEN ?? env.shehzadalgo_GATEWAY_TOKEN;
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    const shouldRemove = await prompter.confirmSkipInNonInteractive({
      message: "Remove legacy gateway services (shehzadalgo/moltbot) now?",
      initialValue: true,
    });
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing ShehzadAlgo gateway next.");
      }
    }
  }
