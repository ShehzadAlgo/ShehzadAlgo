...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "shehzadalgo-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.shehzadalgo_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when shehzadalgo_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.shehzadalgo_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
