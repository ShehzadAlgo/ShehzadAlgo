...
export const resolveShehzadAlgoPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadShehzadAlgoPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/shehzadalgo.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveShehzadAlgoPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.shehzadalgo_STATE_DIR;
  originalUpdateInProgress = process.env.shehzadalgo_UPDATE_IN_PROGRESS;
  process.env.shehzadalgo_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "shehzadalgo-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.shehzadalgo_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.shehzadalgo_UPDATE_IN_PROGRESS;
  } else {
    process.env.shehzadalgo_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
