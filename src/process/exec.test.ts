...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["shehzadalgo_BASE_ENV"]);
    process.env.shehzadalgo_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.shehzadalgo_BASE_ENV ?? "") + "|" + (process.env.shehzadalgo_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { shehzadalgo_TEST_ENV: "ok" },
        },
      );
