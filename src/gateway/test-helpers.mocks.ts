...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `shehzadalgo-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.shehzadalgo_CONFIG_PATH = path.join(root, "shehzadalgo.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "shehzadalgo.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "shehzadalgo-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadShehzadAlgoPlugins: () => pluginRegistryState.registry,
  };
});
