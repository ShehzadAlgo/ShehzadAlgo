...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/shehzadalgo/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/shehzadalgo");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/shehzadalgo/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SHEHZADALGO_CONTROL_UI_BASE_PATH__ = "/shehzadalgo";
    const app = mountApp("/shehzadalgo/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/shehzadalgo");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/shehzadalgo/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "shehzadalgo.control.settings.v1",
