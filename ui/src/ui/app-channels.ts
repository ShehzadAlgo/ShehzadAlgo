...
export async function handleWhatsAppStart(host: ShehzadAlgoApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: ShehzadAlgoApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: ShehzadAlgoApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: ShehzadAlgoApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: ShehzadAlgoApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: ShehzadAlgoApp): string {
...
function resolveGatewayHttpAuthHeader(host: ShehzadAlgoApp): string | null {
...
function buildGatewayHttpHeaders(host: ShehzadAlgoApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: ShehzadAlgoApp) {
...
export function handleNostrProfileToggleAdvanced(host: ShehzadAlgoApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: ShehzadAlgoApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: ShehzadAlgoApp) {
  const state = host.nostrProfileFormState;
