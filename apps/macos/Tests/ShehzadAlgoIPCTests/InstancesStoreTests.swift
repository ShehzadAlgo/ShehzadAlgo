...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: ShehzadAlgoProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ShehzadAlgoProtocol.AnyCodable] = [
            "presence": .init([ShehzadAlgoProtocol.AnyCodable(entry)]),
        ]
        let payload = ShehzadAlgoProtocol.AnyCodable(payloadMap)
