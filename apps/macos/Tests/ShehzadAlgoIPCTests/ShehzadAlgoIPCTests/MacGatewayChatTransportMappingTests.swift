...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: ShehzadAlgoProtocol.AnyCodable(["ok": ShehzadAlgoProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: ShehzadAlgoProtocol.AnyCodable(["ok": ShehzadAlgoProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = ShehzadAlgoProtocol.AnyCodable([
            "runId": ShehzadAlgoProtocol.AnyCodable("run-1"),
            "sessionKey": ShehzadAlgoProtocol.AnyCodable("main"),
            "state": ShehzadAlgoProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: ShehzadAlgoProtocol.AnyCodable(["a": ShehzadAlgoProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
