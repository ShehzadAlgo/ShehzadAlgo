...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.shehzadalgo", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ShehzadAlgoSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ShehzadAlgoSessionPreviewEntry,
    ...
    private static func cache(payload: ShehzadAlgoSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ShehzadAlgoChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ShehzadAlgoKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ShehzadAlgoKit.AnyCodable]) -> [ShehzadAlgoChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ShehzadAlgoChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ShehzadAlgoChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ShehzadAlgoChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ShehzadAlgoChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ShehzadAlgoChatMessage) -> String? {
