...
enum CommandResolver {
    private static let projectRootDefaultsKey = "shehzadalgo.gatewayProjectRootPath"
    private static let helperName = "shehzadalgo"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let shehzadalgoEntry = root.appendingPathComponent("shehzadalgo.mjs").path
        if FileManager().isReadableFile(atPath: shehzadalgoEntry) { return shehzadalgoEntry }
        let binEntry = root.appendingPathComponent("bin/shehzadalgo.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/shehzadalgo")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let shehzadalgoPaths = self.shehzadalgoManagedPaths(home: home)
        if !shehzadalgoPaths.isEmpty {
            extras.insert(contentsOf: shehzadalgoPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + shehzadalgoPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func shehzadalgoManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".shehzadalgo"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func shehzadalgoExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectShehzadAlgoExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("shehzadalgo.mjs").path,
            root.appendingPathComponent("bin/shehzadalgo.js").path,
        ]
    ...
    static func hasAnyShehzadAlgoInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.shehzadalgoExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func shehzadalgoNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let shehzadalgoPath = self.projectShehzadAlgoExecutable(projectRoot: root) {
                return [shehzadalgoPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "shehzadalgo", subcommand] + extraArgs
            }
            if let shehzadalgoPath = self.shehzadalgoExecutable(searchPaths: searchPaths) {
                return [shehzadalgoPath, subcommand] + extraArgs
            }

            let missingEntry = """
            shehzadalgo entrypoint missing (looked for dist/index.js or shehzadalgo.mjs); run pnpm build.
            """
    ...
    static func shehzadalgoCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real shehzadalgo CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/shehzadalgo"
        ...
        \(cliSection)
        \(projectSection)
        if command -v shehzadalgo >/dev/null 2>&1; then
          CLI="$(command -v shehzadalgo)"
          shehzadalgo \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/shehzadalgo.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/shehzadalgo.mjs"
            node "$PRJ/shehzadalgo.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/shehzadalgo.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/shehzadalgo.js"
            node "$PRJ/bin/shehzadalgo.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent shehzadalgo"
          pnpm --silent shehzadalgo \(quotedArgs);
        else
          echo "shehzadalgo CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ShehzadAlgoConfigFile.loadDict()
