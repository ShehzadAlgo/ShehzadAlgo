...
private let webChatSwiftLogger = Logger(subsystem: "ai.shehzadalgo", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: ShehzadAlgoChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ShehzadAlgoChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(ShehzadAlgoChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ShehzadAlgoChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ShehzadAlgoGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    ShehzadAlgoChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    ShehzadAlgoAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ShehzadAlgoChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ShehzadAlgoChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ShehzadAlgoChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: ShehzadAlgoChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ShehzadAlgoChatView>) -> NSViewController
    {
