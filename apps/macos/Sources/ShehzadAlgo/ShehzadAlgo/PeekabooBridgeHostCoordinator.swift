...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.shehzadalgo", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: ShehzadAlgoPeekabooBridgeServices?
    private static var shehzadalgoSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("ShehzadAlgo", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = ShehzadAlgoPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.shehzadalgoSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.shehzadalgoSocketPath, privacy: .public)")
    }
...
@MainActor
private final class ShehzadAlgoPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.shehzadalgo.peekaboo")
