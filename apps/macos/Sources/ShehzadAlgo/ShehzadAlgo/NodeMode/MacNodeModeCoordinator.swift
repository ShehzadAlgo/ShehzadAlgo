...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.shehzadalgo", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: ShehzadAlgoNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [ShehzadAlgoCapability.canvas.rawValue, ShehzadAlgoCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(ShehzadAlgoCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if ShehzadAlgoLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(ShehzadAlgoCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            ShehzadAlgoSystemCommand.which.rawValue,
            ShehzadAlgoSystemCommand.run.rawValue,
            ShehzadAlgoSystemCommand.execApprovalsGet.rawValue,
            ShehzadAlgoSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(ShehzadAlgoCapability.camera.rawValue) {
        ...
        if capsSet.contains(ShehzadAlgoCapability.location.rawValue) {
