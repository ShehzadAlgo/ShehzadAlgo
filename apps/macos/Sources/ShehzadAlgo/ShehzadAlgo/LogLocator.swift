...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SHEHZADALGO_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/shehzadalgo")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("shehzadalgo-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("shehzadalgo-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/shehzadalgo/ (rolling or stdout), or nil if none exist.
