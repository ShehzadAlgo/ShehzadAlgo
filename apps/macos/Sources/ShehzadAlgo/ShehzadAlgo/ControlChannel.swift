...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ShehzadAlgoProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.shehzadalgo", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ShehzadAlgoKit.AnyCodable]()) {
                $0[$1.key] = ShehzadAlgoKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ShehzadAlgoProtocol.AnyCodable?) -> [String: ShehzadAlgoProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ShehzadAlgoProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ShehzadAlgoKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ShehzadAlgoProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("shehzadalgo.control.heartbeat")
    static let controlAgentEvent = Notification.Name("shehzadalgo.control.agent")
}
