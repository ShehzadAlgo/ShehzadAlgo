...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: ShehzadAlgoOAuthStore.AnthropicOAuthStatus = ShehzadAlgoOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: ShehzadAlgoOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = ShehzadAlgoOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = ShehzadAlgoOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try ShehzadAlgoOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. ShehzadAlgo can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: ShehzadAlgoOAuthStore.AnthropicOAuthStatus,
