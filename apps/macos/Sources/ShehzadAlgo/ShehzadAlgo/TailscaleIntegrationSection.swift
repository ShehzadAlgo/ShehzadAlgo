...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.shehzadalgo/shehzadalgo.json. Prefer SHEHZADALGO_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.shehzadalgo/shehzadalgo.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.shehzadalgo/shehzadalgo.json. Restart the gateway to apply."
        }
