...
private let deepLinkLogger = Logger(subsystem: "ai.shehzadalgo", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "ShehzadAlgo is paused", message: "Unpause ShehzadAlgo to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run ShehzadAlgo agent?", message: body) else { return }
        }
