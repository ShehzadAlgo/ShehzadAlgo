...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.shehzadalgo.mac`) even if the shipped
    ...
    {
        if environment["SHEHZADALGO_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "shehzadalgo.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "shehzadalgo.nixMode") { return true }
