...
enum ShehzadAlgoLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = ShehzadAlgoOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ShehzadAlgoFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ShehzadAlgoLogging.bootstrapIfNeeded()
        let label = ShehzadAlgoLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ShehzadAlgoOSLogHandler: LogHandler {
...
struct ShehzadAlgoFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ShehzadAlgoLogging.parseLabel(self.label)
