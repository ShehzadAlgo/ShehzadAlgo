...
enum DebugActions {
    private static let verboseDefaultsKey = "shehzadalgo.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "shehzadalgo.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = ShehzadAlgoPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "ShehzadAlgo", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = ShehzadAlgoPaths.configURL
        guard
