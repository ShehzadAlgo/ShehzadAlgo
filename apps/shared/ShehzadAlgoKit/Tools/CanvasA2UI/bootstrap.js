...
import "@a2ui/lit/ui";
import { themeContext } from "@shehzadalgo/a2ui-theme-context";
...
class ShehzadAlgoA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--shehzadalgo-a2ui-inset-top, 0px)
        var(--shehzadalgo-a2ui-inset-right, 0px)
        var(--shehzadalgo-a2ui-inset-bottom, 0px)
        var(--shehzadalgo-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--shehzadalgo-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--shehzadalgo-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--shehzadalgo-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--shehzadalgo-a2ui-empty-top, var(--shehzadalgo-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.shehzadalgoA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["shehzadalgo:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__shehzadalgoLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.shehzadalgoCanvasA2UIAction ??
      globalThis.shehzadalgoCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.shehzadalgoCanvasA2UIAction) {
...
if (!customElements.get("shehzadalgo-a2ui-host")) {
  customElements.define("shehzadalgo-a2ui-host", ShehzadAlgoA2UIHost);
}
