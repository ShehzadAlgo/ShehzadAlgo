...
import Foundation
import Testing
@testable import ShehzadAlgoChatUI
...
private final class TestChatTransport: @unchecked Sendable, ShehzadAlgoChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ShehzadAlgoChatHistoryPayload]
    private let sessionsResponses: [ShehzadAlgoChatSessionsListResponse]

    private let stream: AsyncStream<ShehzadAlgoChatTransportEvent>
    private let continuation: AsyncStream<ShehzadAlgoChatTransportEvent>.Continuation

    init(
        historyResponses: [ShehzadAlgoChatHistoryPayload],
        sessionsResponses: [ShehzadAlgoChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ShehzadAlgoChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ShehzadAlgoChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ShehzadAlgoChatHistoryPayload(
        ...
        attachments _: [ShehzadAlgoChatAttachmentPayload]) async throws -> ShehzadAlgoChatSendResponse
    {
        ...
        return ShehzadAlgoChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ShehzadAlgoChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ShehzadAlgoChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ShehzadAlgoChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ShehzadAlgoChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ShehzadAlgoChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { ShehzadAlgoChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = ShehzadAlgoChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = ShehzadAlgoChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = ShehzadAlgoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ShehzadAlgoChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
