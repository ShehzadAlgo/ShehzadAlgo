...
private let chatUILogger = Logger(subsystem: "ai.shehzadalgo", category: "ShehzadAlgoChatUI")

@MainActor
@Observable
public final class ShehzadAlgoChatViewModel {
    public private(set) var messages: [ShehzadAlgoChatMessage] = []
    ...
    public var attachments: [ShehzadAlgoPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ShehzadAlgoChatPendingToolCall] = []
    public private(set) var sessions: [ShehzadAlgoChatSessionEntry] = []
    private let transport: any ShehzadAlgoChatTransport
    ...
    private var pendingToolCallsById: [String: ShehzadAlgoChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any ShehzadAlgoChatTransport) {
    ...
    public var sessionChoices: [ShehzadAlgoChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [ShehzadAlgoChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ShehzadAlgoPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ShehzadAlgoChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ShehzadAlgoChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func messageIdentityKey(for message: ShehzadAlgoChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [ShehzadAlgoChatMessage],
        incoming: [ShehzadAlgoChatMessage]) -> [ShehzadAlgoChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [ShehzadAlgoChatMessage]) -> [ShehzadAlgoChatMessage] {
        var result: [ShehzadAlgoChatMessage] = []
    ...
    private static func dedupeKey(for message: ShehzadAlgoChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ShehzadAlgoChatMessageContent] = [
            ShehzadAlgoChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> ShehzadAlgoChatAttachmentPayload in
            ShehzadAlgoChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ShehzadAlgoChatSessionEntry {
        ShehzadAlgoChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: ShehzadAlgoChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ShehzadAlgoChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ShehzadAlgoAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ShehzadAlgoChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ShehzadAlgoPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
