...
@MainActor
struct ShehzadAlgoChatComposer: View {
    @Bindable var viewModel: ShehzadAlgoChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ShehzadAlgoChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ShehzadAlgoChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ShehzadAlgoPendingAttachment.id)
                { (att: ShehzadAlgoPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ShehzadAlgoPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(ShehzadAlgoChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ShehzadAlgoChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ShehzadAlgoChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ShehzadAlgoChatTheme.subtleCard)
        .clipShape(Capsule())
    }
