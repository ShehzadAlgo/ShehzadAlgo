...
struct ChatMessageBubble: View {
    let message: ShehzadAlgoChatMessage
    let style: ShehzadAlgoChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? ShehzadAlgoChatTheme.userText : ShehzadAlgoChatTheme.assistantText
    ...
    private var inlineAttachments: [ShehzadAlgoChatMessageContent] {
    ...
    private var toolCalls: [ShehzadAlgoChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [ShehzadAlgoChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? ShehzadAlgoChatTheme.userBubble
        }
        if self.style == .onboarding {
            return ShehzadAlgoChatTheme.onboardingAssistantBubble
        }
        return ShehzadAlgoChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return ShehzadAlgoChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: ShehzadAlgoChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? ShehzadAlgoChatTheme.userText : ShehzadAlgoChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: ShehzadAlgoChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ShehzadAlgoChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(ShehzadAlgoChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [ShehzadAlgoChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: ShehzadAlgoChatTheme.assistantText)
            }
        }
    }
}
