...
enum ShehzadAlgoChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("ShehzadAlgoChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("ShehzadAlgoChatTheme.onboardingAssistantBubble"),
...
enum ShehzadAlgoPlatformImageFactory {
    static func image(_ image: ShehzadAlgoPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
