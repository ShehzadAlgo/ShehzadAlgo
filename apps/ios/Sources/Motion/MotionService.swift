...
final class MotionService: MotionServicing {
    func activities(params: ShehzadAlgoMotionActivityParams) async throws -> ShehzadAlgoMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[ShehzadAlgoMotionActivityEntry], Error>) in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        ShehzadAlgoMotionActivityEntry(
        ...
        return ShehzadAlgoMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: ShehzadAlgoPedometerParams) async throws -> ShehzadAlgoPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<ShehzadAlgoPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = ShehzadAlgoPedometerPayload(
