...
struct IOSGatewayChatTransport: ShehzadAlgoChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.shehzadalgo", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> ShehzadAlgoChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ShehzadAlgoChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ShehzadAlgoChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ShehzadAlgoChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ShehzadAlgoChatAttachmentPayload]) async throws -> ShehzadAlgoChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [ShehzadAlgoChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(ShehzadAlgoChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(ShehzadAlgoGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ShehzadAlgoGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ShehzadAlgoChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ShehzadAlgoAgentEventPayload.self)
                        {
