...
final class ContactsService: ContactsServicing {
    ...
    func search(params: ShehzadAlgoContactsSearchParams) async throws -> ShehzadAlgoContactsSearchPayload {
        ...
        return ShehzadAlgoContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: ShehzadAlgoContactsAddParams) async throws -> ShehzadAlgoContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return ShehzadAlgoContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return ShehzadAlgoContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> ShehzadAlgoContactPayload {
        ShehzadAlgoContactPayload(
