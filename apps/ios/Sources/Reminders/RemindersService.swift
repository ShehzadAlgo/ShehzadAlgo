...
final class RemindersService: RemindersServicing {
    func list(params: ShehzadAlgoRemindersListParams) async throws -> ShehzadAlgoRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[ShehzadAlgoReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return ShehzadAlgoReminderPayload(
        ...
        return ShehzadAlgoRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: ShehzadAlgoRemindersAddParams) async throws -> ShehzadAlgoRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = ShehzadAlgoReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return ShehzadAlgoRemindersAddPayload(reminder: payload)
    }
