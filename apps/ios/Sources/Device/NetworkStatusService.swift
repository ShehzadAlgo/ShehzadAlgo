...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> ShehzadAlgoNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> ShehzadAlgoNetworkStatusPayload {
        let status: ShehzadAlgoNetworkPathStatus = switch path.status {
        ...
        var interfaces: [ShehzadAlgoNetworkInterfaceType] = []
        if path.usesInterfaceType(.wifi) { interfaces.append(.wifi) }
        if path.usesInterfaceType(.cellular) { interfaces.append(.cellular) }
        if path.usesInterfaceType(.wiredEthernet) { interfaces.append(.wired) }
        if interfaces.isEmpty { interfaces.append(.other) }

        return ShehzadAlgoNetworkStatusPayload(
            status: status,
    ...
    private static func fallbackPayload() -> ShehzadAlgoNetworkStatusPayload {
        ShehzadAlgoNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
