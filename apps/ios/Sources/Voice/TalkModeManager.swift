...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__SHEHZADALGO_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<ShehzadAlgoTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = ShehzadAlgoTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> ShehzadAlgoTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return ShehzadAlgoTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> ShehzadAlgoTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> ShehzadAlgoTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return ShehzadAlgoTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> ShehzadAlgoTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = ShehzadAlgoTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: ShehzadAlgoTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: ShehzadAlgoAgentEventPayload.self) else {
                continue
            }
