---
description: Update shehzadalgo from upstream when branch has diverged (ahead/behind)
---

# shehzadalgo Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "shehzadalgo" || true

# Move old version
mv /Applications/shehzadalgo.app /tmp/shehzadalgo-backup.app

# Install new build
cp -R dist/shehzadalgo.app /Applications/

# Launch
open /Applications/shehzadalgo.app
```
...
pnpm shehzadalgo health

# Verify no zombie processes
ps aux | grep -E "(shehzadalgo|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm shehzadalgo agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update shehzadalgo.json with fallback chains
...
pnpm shehzadalgo agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
