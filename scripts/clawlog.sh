...
show_usage() {
    cat << EOF
clawlog - ShehzadAlgo Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View ShehzadAlgo logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    ShehzadAlgo logs flow through the macOS unified log (subsystem: ai.shehzadalgo).
