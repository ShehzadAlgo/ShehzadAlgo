...
BASE_IMAGE="${BASE_IMAGE:-shehzadalgo-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-shehzadalgo-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=shehzadalgo.sandbox=1)
NOTE
