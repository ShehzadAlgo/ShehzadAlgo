...
# ShehzadAlgo Auth Widget for Termux
...
# Check auth status
termux-toast "Checking ShehzadAlgo auth..."

STATUS=$(ssh "$SERVER" '$HOME/shehzadalgo/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/shehzadalgo/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|SHEHZADALGO_EXPIRING|shehzadalgo_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/shehzadalgo/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|SHEHZADALGO_EXPIRED|shehzadalgo_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: ShehzadAlgo won't work until re-auth"
                ;;
        esac
        ;;
