...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/shehzadalgo/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "ShehzadAlgo Re-Auth" -c "After getting key, run: ssh $SERVER '~/shehzadalgo/scripts/mobile-reauth.sh'" --id shehzadalgo-auth
        ;;
    *)
