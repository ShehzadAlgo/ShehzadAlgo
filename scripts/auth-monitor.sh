...
#
# Suggested cron: */30 * * * * /home/admin/shehzadalgo/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send ShehzadAlgo notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., shehzadalgo-alerts)
...
STATE_FILE="$HOME/.shehzadalgo/auth-monitor-state"
...
send_notification() {
    ...
    # Send via ShehzadAlgo if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use shehzadalgo
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via ShehzadAlgo to $NOTIFY_PHONE..."
            shehzadalgo send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: ShehzadAlgo Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! ShehzadAlgo is down. Run: ssh l36 '~/shehzadalgo/scripts/mobile-reauth.sh'" "urgent"
    exit 1
