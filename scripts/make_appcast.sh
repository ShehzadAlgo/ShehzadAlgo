...
ZIP=${1:?"Usage: $0 ShehzadAlgo-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/Shehzadwattu523/ShehzadAlgo/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^ShehzadAlgo-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/shehzadalgo/shehzadalgo/releases/download/v${VERSION}/"}
