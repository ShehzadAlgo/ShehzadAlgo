...
# Build and bundle ShehzadAlgo into a minimal .app we can open.
# Outputs to dist/ShehzadAlgo.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/ShehzadAlgo.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="ShehzadAlgo"
BUNDLE_ID="${BUNDLE_ID:-ai.shehzadalgo.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/Shehzadwattu523/ShehzadAlgo/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/ShehzadAlgo/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ShehzadAlgoBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ShehzadAlgoGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/ShehzadAlgo"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/ShehzadAlgo"
fi
chmod +x "$APP_ROOT/Contents/MacOS/ShehzadAlgo"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/ShehzadAlgo" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/ShehzadAlgo/Resources/ShehzadAlgo.icns" "$APP_ROOT/Contents/Resources/ShehzadAlgo.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/ShehzadAlgo/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ShehzadAlgoKit resources"
SHEHZADALGOKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ShehzadAlgoKit_ShehzadAlgoKit.bundle"
if [ -d "$SHEHZADALGOKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ShehzadAlgoKit_ShehzadAlgoKit.bundle"
  cp -R "$SHEHZADALGOKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ShehzadAlgoKit_ShehzadAlgoKit.bundle"
else
  echo "WARN: ShehzadAlgoKit resource bundle not found at $SHEHZADALGOKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running ShehzadAlgo"
killall -q ShehzadAlgo 2>/dev/null || true
