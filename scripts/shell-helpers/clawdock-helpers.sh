#!/usr/bin/env bash
# ClawDock - Docker helpers for ShehzadAlgo
# Inspired by Simon Willison's "Running ShehzadAlgo in Docker"
# https://til.simonwillison.net/llms/shehzadalgo-docker
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/Shehzadwattu523/ShehzadAlgo/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
# Common paths to check for ShehzadAlgo
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/shehzadalgo"
  "${HOME}/workspace/shehzadalgo"
  "${HOME}/projects/shehzadalgo"
  "${HOME}/dev/shehzadalgo"
  "${HOME}/code/shehzadalgo"
  "${HOME}/src/shehzadalgo"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found ShehzadAlgo at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/shehzadalgo"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ ShehzadAlgo not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/Shehzadwattu523/ShehzadAlgo.git ~/shehzadalgo"
    echo "  cd ~/shehzadalgo && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^SHEHZADALGO_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d shehzadalgo-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart shehzadalgo-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f shehzadalgo-gateway
}
...
clawdock-workspace() {
  cd ~/.shehzadalgo/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec shehzadalgo-gateway \
    bash -c 'echo "alias shehzadalgo=\"./shehzadalgo.mjs\"" > /tmp/.bashrc_shehzadalgo && bash --rcfile /tmp/.bashrc_shehzadalgo'
}
...
clawdock-cli() {
  _clawdock_compose run --rm shehzadalgo-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build shehzadalgo-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "SHEHZADALGO_GATEWAY_TOKEN=$token" shehzadalgo-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" shehzadalgo-gateway \
    bash -c './shehzadalgo.mjs config set gateway.remote.token "$TOKEN" && ./shehzadalgo.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec shehzadalgo-gateway \
    bash -c "./shehzadalgo.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart shehzadalgo-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm shehzadalgo-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec shehzadalgo-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'shehzadalgo config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for ShehzadAlgo${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (shehzadalgo alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to shehzadalgo project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.shehzadalgo)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'shehzadalgo channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'shehzadalgo status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.shehzadalgo.ai${_CLR_RESET}"
  echo ""
}
