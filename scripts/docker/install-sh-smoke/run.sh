...
INSTALL_URL="${SHEHZADALGO_INSTALL_URL:-https://shehzadalgo.ai/install.sh}"
SMOKE_PREVIOUS_VERSION="${SHEHZADALGO_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SHEHZADALGO_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="shehzadalgo"
PACKAGE_NAME="${SHEHZADALGO_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SHEHZADALGO_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SHEHZADALGO_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SHEHZADALGO_INSTALL_LATEST_OUT:-}"
fi
