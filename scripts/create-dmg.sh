...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "ShehzadAlgo")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "ShehzadAlgo")
...
DMG_TEMP="$(mktemp -d /tmp/shehzadalgo-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/ShehzadAlgo/Resources/ShehzadAlgo.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/shehzadalgo-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/shehzadalgo-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/shehzadalgo-dmg-limits.txt
