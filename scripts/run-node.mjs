...
const shouldBuild = (deps) => {
  if (deps.env.SHEHZADALGO_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.SHEHZADALGO_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[shehzadalgo] ${message}\n`);
};

const runShehzadAlgo = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["shehzadalgo.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runShehzadAlgo(deps);
  }
