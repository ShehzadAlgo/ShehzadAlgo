...
const urlRaw = getArg("--url") ?? process.env.SHEHZADALGO_GATEWAY_URL;
const token = getArg("--token") ?? process.env.SHEHZADALGO_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.SHEHZADALGO_RUN_DANGEROUS === "1";
...
if (!urlRaw || !token) {
  ...
  console.error(
    "Usage: bun scripts/dev/ios-node-e2e.ts --url <wss://host[:port]> --token <gateway.auth.token> [--node <id|name-substring>] [--dangerous] [--json]\n" +
      "Or set env: SHEHZADALGO_GATEWAY_URL / SHEHZADALGO_GATEWAY_TOKEN",
  );
  process.exit(1);
}
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "shehzadalgo ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "shehzadalgo-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "ShehzadAlgo E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",
