 ...
 * `shehzadalgo update`. Use it to verify the completion installation flow
...
\`shehzadalgo update\`, \`shehzadalgo doctor\`, and \`shehzadalgo onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# ShehzadAlgo Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`shehzadalgo completion --install\` later to enable.`));
    return;
  }
