...
IMAGE_NAME="shehzadalgo-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="shehzadalgo-net-e2e-$$"
GW_NAME="shehzadalgo-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "SHEHZADALGO_GATEWAY_TOKEN=$TOKEN" \
	  -e "SHEHZADALGO_SKIP_CHANNELS=1" \
	  -e "SHEHZADALGO_SKIP_GMAIL_WATCHER=1" \
	  -e "SHEHZADALGO_SKIP_CRON=1" \
	  -e "SHEHZADALGO_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
