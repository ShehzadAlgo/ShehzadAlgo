#!/bin/bash
# Setup ShehzadAlgo Auth Management System
...
echo "=== ShehzadAlgo Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'shehzadalgo-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  ShehzadAlgo message: Send warning via ShehzadAlgo itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/shehzadalgo-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/shehzadalgo-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/shehzadalgo-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now shehzadalgo-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status shehzadalgo-auth-monitor.timer"
echo ""
echo "Quick commands:"
